...
internal val developerHelp: Map<String, HelpContent> = mapOf(
    "developer_select_model" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Loading state", "Tapping the \"...\" button next to Model on the API Test page fetches the list on the spot and opens this picker once it lands — there's no separate Fetch step. An empty list here means the fetch call returned zero models (a fetch error shows its message instead)."),
    ...
    "refresh_result" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Back", "Returns to the Info Providers sub-page. This screen is a one-time snapshot taken when the refresh finished — it doesn't update live.")
        )
    ),
    "share_target" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("New Report card", "📝. Routes to the New Report flow — text becomes the prompt; the first image attaches for vision; non-image files queue for one-tap auto-attach as a knowledge base on the New Report screen. Greyed when there's neither text nor URIs."),
...
,
    "test" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Stress test", "Exercises the whole report pipeline at volume: fires one AI report per Example Prompt, all at once, each using the models of the swarm named \"Level 2\". Fire-and-forget — the reports generate concurrently in the background and existing runtime data is left untouched."),
        )
    ),
    ...
    "test_all_models_l1" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Probe budget", "Requests use a 64-token limit and a 60-second timeout. Costs are estimates; providers can exceed requested token limits or charge for internal tools. Groq Compound is skipped before dispatch because it exceeded this budget. A probe costing more than 5 cents is excluded from later sweeps."),
    ...
    "trace_list" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Pitfalls", "Cohere's rerank endpoint lives on a second host (api.cohere.com) besides its OpenAI-compat baseUrl — matched via the per-provider `auxHosts` field (set in assets/providers/Cohere.json), which is what the Provider filter and Model picker resolve against. Without an `auxHosts` entry those traces would land in \"(unknown)\". Add `auxHosts` to any provider's JSON for a similar second-host case."),
        )
    ),
    "trace_detail" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Agent button", "Only shown when a saved Agent matches the trace's (provider, model) pair. Drills into the agent's edit screen. Provider drill-in lives on the title-bar ℹ️ instead — directly when there is no model, or via Model Info → Provider when the trace carries a model."),
            ...
            HelpCard("View selector", "Meta / Get / Req Hdr / Rsp Hdr / Req / Rsp. Get is only present when the request URL had query parameters — same renderer as the header views. Active view highlights blue."),
            HelpCard("Bottom action row", "Prev/next between traces is a swipe left/right on the content area, not a button — it resets back to the Rsp tab on each step. The row below carries non-navigation actions only: a 📝/👁 pair (only when the trace is report-scoped) to jump to that report's Manage or View screen, plus an \"Edit\" button."),
            HelpCard("Edit", "Persists the trace's request body + a redacted url + model into eval_prefs, then opens EditApiRequestScreen with that JSON loaded — fast \"replay this request\" path."),
            HelpCard("Title bar — ℹ️", "Three-way, in order: (1) info-provider catalog fetches (LiteLLM / models.dev / Helicone / llm-prices / Artificial Analysis / OpenRouter's pricing endpoint / HuggingFace) open that provider's help topic instead; (2) a trace with a model opens Model Info for (provider, model); (3) a trace with only a provider match (e.g. a /v1/models list call) opens the Provider edit screen in AI Setup."),
            HelpCard("Title bar — 📋 / 📤", "Copy and Share use a redacted variant of the active tab's bytes (URL query params, sensitive headers, sensitive JSON keys → [REDACTED]) so secrets don't leave the device — the on-screen tree itself always shows raw bytes."),
    ...
    "developer_test" to HelpContent(
        title = "Help - API Test",
        cards = listOf(
            HelpCard("Overview", "Developer fixture for hand-crafting an API call. Selects provider/endpoint/key/model/prompt/system/temperature/max_tokens, persists everything except the API key to eval_prefs (the key stays in an in-memory-only holder, never written to disk), and opens Edit Request to send a real call."),
            ...
            HelpCard("Build Request", "Button at the bottom of the form. Clears any previous raw_json so EditApiRequestScreen builds fresh JSON from the form. Always remembers everything else for next session."),
    ...
    "developer_edit" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Pitfalls", "API key for the call comes from an in-memory-only holder (deliberately excluded from eval_prefs so a raw key never sits in cleartext SharedPreferences or a backup zip) and is sent in plain text via the standard provider Authorization scheme; if you mis-typed it on the test screen, or the app process was killed and restarted since, the failure looks like a 401 in the trace, not a UI error here."),
        )
    ),
    "applog_list" to HelpContent(
        title = "Help - Application log — file list",
        cards = listOf(
            HelpCard("Overview", "List of daily-rotating application log files stored under `<filesDir>/applog/`. Each file captures one calendar day filtered by the level threshold set in Settings → Log/trace/audit/statistics. Rows show the date (YYYY-MM-DD) and on-disk size."),
            ...
            HelpCard("Title bar — 📈", "Jumps to App log statistics — aggregate counts across every stored file."),
            HelpCard("Delete > 7 days button", "Below the list — a one-tap batch trim, after confirmation, of every log file whose last-modified time is more than 7 days old. Independent of the title-bar 🗑, which clears everything."),
    ...
    "applog_detail" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Title bar — Copy / Share / 🧽 / 🗑", "Copy and Share open a chooser dialog (last N lines / complete log / filtered-only). 🧽 resets every filter in one tap — hidden unless at least one differs from the default. 🗑 deletes the entire file with confirmation — back returns to the file list with the deletion already reflected."),
    ...
    "audit_list" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Row", "Shows the report's live title (or *(deleted report)* when the report's been removed — the audit survives deletion), the short report id, the last-activity time and the line count. Rows alternate background shade for readability."),
            HelpCard("Title bar — 🧾 / title", "Tapping the 🧾 icon or the screen title jumps back to the Monitor hub."),
    ...
    "audit_detail" to HelpContent(
        ...
        cards = listOf(
            HelpCard("Overview", "The full audit trail for one report, oldest-first. The first line is always *Start AI report with internal id: <UUID>*. Each line is timestamped `yyyy-MM-dd HH:mm:ss.SSS`; rows alternate background shade."),
            ...
            HelpCard("Tap an API-call line", "Tapping a technical or functional API-call line opens that call's API Trace detail (matched by timestamp). Requires tracing to be enabled — otherwise a toast says no trace was recorded. Non-API lines (start/end, user actions) aren't tappable."),
            HelpCard("Line colour", "Dim grey = API technical line · 🔴 red = API error · 🔵 blue = batch start/end · 🟠 orange = a deletion. Everything else is near-white."),
            HelpCard("Title bar — 🧾 / title", "Tapping the 🧾 icon or the screen title jumps to the Monitor hub. Copy / Share grab the whole trail. 🗑 deletes this report's audit file (the report itself is untouched)."),
    ...
    "external_intent" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Prompts and data", "<prompt>text</prompt> and <system>text</system> always contain literal text, even when it matches a saved prompt name or ID. Matching @name@ placeholders use supplied data. Only <parameters>Name</parameters> looks up a Parameters preset by ID or unique name. Eval sends data tags only for placeholders used in its instruction text. <default>, <model> and <edit> are no longer controls."),
            ...
            HelpCard("Will-do card", "Shows whether confirmation will generate immediately or open model selection. Lists the report type and requested Agents, Flocks or Swarms; otherwise you choose models on the next screen."),
            ...
            HelpCard("Confirm", "Bottom-right button reads *Generate* when the intent will auto-fire (report type + at least one Agent/Flock/Swarm, no select flag) and *Continue* otherwise. Either commits: the auto-generate path fires the report immediately; model selection opens with the supplied prompt. Side-effects only fire once generation actually happens."),
    ...
    "inaccessible_models" to HelpContent(
        ...
        cards = listOf(
            HelpCard("Overview", "Curated list of model definitions that your account genuinely can't reach — no API key, capability mismatch, Together's non-serverless tier, geo-blocked endpoints, etc. Entries are dimmed in pickers; known entries are skipped entirely (not re-probed) on the next Test All Models run."),
            HelpCard("Title bar", "Back / 🏠 as usual, plus 🆕 to add an entry by hand and 🧹 to jump to the Test hub."),
            HelpCard("Rows", "One row per `(provider, model)` pair, shown as \"provider · model\" — the reason isn't shown on the row, only on the row's detail screen. Sorted by the `provider:model` key, case-insensitive."),
            HelpCard("View / Edit / Delete", "Tap a row to open its detail screen (provider, model, reason). From there: Edit opens a form with a model picker plus a free-text Reason field; Copy prefills a new entry from this one; Delete asks for confirmation first."),
            HelpCard("Auto-population", "Two sources feed this list without user action. (1) `assets/inaccessible.json` ships a curated seed merged in on every app start (existing entries are left alone; a missing reason defaults to \"Unable to access non-serverless (bundled)\"). (2) The Test All Models engine adds/updates an entry live whenever a probe's error contains \"non-serverless\", \"is not available on\", or a plain HTTP 404 — that probe still counts as a PASS in the run, not a FAIL. You can also add/edit entries by hand."),
            ...
            HelpCard("Reached from", "AI Setup → Models → Inaccessible models.")
        )
    ),
)
