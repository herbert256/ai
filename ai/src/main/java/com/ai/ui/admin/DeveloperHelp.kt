...
,
    ...
    "external_intent" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Saved settings", "Use <system>Name</system> to select a saved system prompt, <prompt>Name</prompt> to select an Example or Internal Prompt, <parameters>Name</parameters> to select a Parameters preset, and <default>Name</default> to select a default prompt for models when no prompt text was supplied. Names ignore case; stable IDs also work. If <system> does not resolve, its content becomes literal system-prompt text. If <prompt> does not resolve, its content becomes the normal report prompt and the saved-prompt picker is skipped. Unresolved Parameters or Default prompt names must be corrected before continuing. Matching @name@ placeholders in normal, system and default prompts use entries from the external instructions."),
