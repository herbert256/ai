...
internal val glossaryHelp: Map<String, HelpContent> = mapOf(
    ...
    "help_glossary_blocks" to HelpContent(
        ...
        cards = listOf(
            HelpCard("Provider", "A vendor or runtime the app can talk to. 35 cloud providers ship with the app, and custom providers can be added under Settings → AI Setup → Providers. The synthetic Local provider appears when on-device LLMs are installed. Each provider carries its API key/state, base URL or runtime identity, throttle caps, retry policy, endpoints, pricing hooks, and model catalog."),
