...
internal val glossaryHelp: Map<String, HelpContent> = mapOf(
    ...
    "concepts" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("App-wide background broken-work scan", "Once on app start and then every 30 seconds the app scans every report modified in the last 7 days for interrupted work — translation placeholders that never converged, fan-out pairs / tournament matches / judge cells the previous session abandoned, single-Meta calls left in PENDING, and stalled regenerate batches. The scan is read-only: it no longer auto-resumes anything. Instead each affected report is listed on the ⚠️ Broken-work screen (the top-bar AI logo turns into a ⚠️ while anything is broken) so you can open it and Regenerate / retry manually. Logs land under the BrokenScan tag. Reports older than 7 days are skipped (out of the active-work window)."),
    ...
    "help_glossary_blocks" to HelpContent(
        ...
        cards = listOf(
            HelpCard("Provider", "A vendor or runtime the app can talk to. 43 cloud providers ship with the app, and custom providers can be added under Settings → AI Setup → Providers. The synthetic Local provider appears when on-device LLMs are installed. Each provider carries its API key/state, base URL or runtime identity, throttle caps, retry policy, endpoints, pricing hooks, and model catalog."),
    ...
    "help_glossary_operations" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Tournament", "Pairwise head-to-head judging over report answers. Each unordered pair is judged in both directions to reduce position bias, then the View screen can aggregate the same match data through Copeland, Elo, Davidson, Markov, Schulze, Colley, or TrueSkill2 rankings."),
    ...
    "help_translations" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Interrupted runs are flagged, not auto-fixed", "Translation runs interrupted by an app restart, a coroutine cancellation, or a mid-flight kill are detected by the 30-second app-wide background scan across reports modified in the last 7 days and listed on the ⚠️ Broken-work screen. The app no longer re-dispatches them on its own — open the report and Restart / retry the run manually. (A 10-second sanity poll on the result page still reconciles a stalled in-memory run whose disk rows already completed, so its hourglass clears.)"),
