...
internal val HELP_TOPICS: Map<String, HelpContent> = mapOf(
    ...
    "internal_prompt_icon_detail" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Model", "Which (provider, model) produced the displayed emoji. For the initial generation this is the bundled DeepSeek (or whatever the user pinned the internal/meta_icon prompt to). After a Find-alternative-icons pick it's the candidate model the user committed to."),
            HelpCard("Prompt — internal/meta_icon", "The resolved bundled prompt that was sent to the model: a fixed 'Please give a fitting emoji for below text…' instruction followed by `@NAME@` and `@TITLE@` substituted with the Internal Prompt's name + title."),
            ...
            HelpCard("Find alternative icons", "Bottom button — opens the model picker. Pick a set of (provider, model) pairs, tap Find Icons; each runs the bundled internal/meta_icon_alt prompt (a variant of meta_icon that adds an explicit 'pick something distinct that still fits' instruction so the candidates differ from the obvious initial emoji). Results land on the Alternative icons live list. Tap a returned emoji to commit it as the new displayed value — the InternalPromptIconCache entry is overwritten with that call's provider/model/prompt/response."),
    ...
    "translation_icon_detail" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Find alternative icons", "Bottom button — opens the model picker. Pick a set of (provider, model) pairs, tap Find Icons; each runs the bundled internal/translation_icon_alt prompt (a variant of translation_icon that adds an explicit 'do not use a country flag emoji' instruction so the candidates aren't all flags). Results land on the Alternative icons live list. Tap a returned emoji to commit it as the new displayed value — the cache entry is overwritten with that call's provider/model/prompt/response."),
