...
internal val HELP_TOPICS: Map<String, HelpContent> = mapOf(
    ...
    "report_result_generation" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Create → Report icons", "Fires the bundled internal/report_icon prompt once per successful agent row in this report, each call against the agent's OWN (provider, model). The template carries two placeholders: @PROMPT@ → the report's prompt (uniform across all agents), @RESPONSE@ → that agent's responseBody (per-agent). On success, the row's leftmost ✅ swaps to the returned emoji, which becomes its own tap target for a per-agent icon detail. The per-call cost folds into the row's existing cost cell. Re-running wipes prior per-agent icons before kicking off."),
    ...
    "agent_icon_detail" to HelpContent(
        title = "Agent icon",
        cards = listOf(
            HelpCard("Overview", "Detail view for one agent's per-row emoji, reached by tapping the leftmost icon cell on a row that Create → Report icons has populated. Same three-card layout as the report-level Icon detail — Model / Prompt / Response — only the data is per-agent."),
            HelpCard("Model", "The agent's own (provider, model). Not the bundled icon-prompt agent — Report icons fires each call against the same model that produced the response we're iconifying."),
            HelpCard("Prompt", "The internal/report_icon prompt template with two substitutions: @PROMPT@ → the report's prompt (the question), @RESPONSE@ → this agent's responseBody (the answer). Same question across rows, different response per row — that's why each row can end up with a different icon."),
            ...
            HelpCard("Cost", "Per-agent icon-call cost only. The row's right-side cost cell on the result screen already folds this into the agent's primary-call cost so the user sees a single total per row."),
            HelpCard("Find alternative icons", "Bottom button — same shape as the report-level Icon detail's. Opens the model picker; pick a fresh set of (provider, model) pairs and tap Find Icons to fan the internal/report_icon prompt out across them (same dual substitution: @PROMPT@ = report prompt, @RESPONSE@ = THIS agent's response). Each result lands on the Alternative icons live list — tap a returned emoji to commit it to THIS agent only. The row's leftmost cell updates to match. Costs from every call (picked or not) accumulate onto the agent's icon-cost field."),
            ...
            HelpCard("Failure path", "When the icon call fails, the row keeps its ✅ — the agent itself succeeded, only the secondary icon call didn't. The failure reason lives here on the per-agent detail screen; re-running Create → Report icons retries, as does Find alternative icons.")
        )
    ),
