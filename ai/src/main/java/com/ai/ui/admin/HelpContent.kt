...
internal val HELP_TOPICS: Map<String, HelpContent> = mapOf(
    ...
    "agent_icon_detail" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Find alternative icons", "Bottom button — opens the model picker. Pick a fresh set of (provider, model) pairs, tap Find Icons; each one runs icons/report_alt (a variant of icons/report that adds an explicit 'pick something distinct' instruction so the candidates differ from the obvious initial emoji). Results land on the Alternative icons live list. Tap a returned emoji to commit it to THIS agent only — the iconWinningTier flag clears (the pick is manual, not a chain result)."),
    ...
    "internal_prompt_icon_detail" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Model", "Which (provider, model) produced the displayed emoji. For the initial generation this is the bundled DeepSeek (or whatever the user pinned the icons/meta prompt to). After a Find-alternative-icons pick it's the candidate model the user committed to."),
            HelpCard("Prompt — icons/meta", "The resolved bundled prompt that was sent to the model: a fixed 'Please give a fitting emoji for below text…' instruction followed by `@NAME@` and `@TITLE@` substituted with the Internal Prompt's name + title."),
            ...
            HelpCard("Find alternative icons", "Bottom button — opens the model picker. Pick a set of (provider, model) pairs, tap Find Icons; each runs the bundled icons/meta_alt prompt (a variant of icons/meta that adds an explicit 'pick something distinct that still fits' instruction so the candidates differ from the obvious initial emoji). Results land on the Alternative icons live list. Tap a returned emoji to commit it as the new displayed value — the InternalPromptIconCache entry is overwritten with that call's provider/model/prompt/response."),
    ...
    "translation_icon_detail" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Model", "Which (provider, model) produced the displayed emoji. Initial generation runs against the bundled `icons/translation` prompt's pinned agent (DeepSeek by default). After a Find-alternative-icons pick this becomes the candidate model the user committed to."),
            HelpCard("Prompt — icons/translation", "The resolved bundled prompt sent to the model: a fixed 'Please give a fitting emoji for …, give only this emoji as feedback, nothing more !!!' instruction with `@LANGUAGE@` substituted with the run's target language (English name like Dutch / Japanese / etc.)."),
            HelpCard("Response", "Large emoji at the top is the resolved one-glyph cache value. Below it (when the model returned more than a single glyph) is the raw response text for context — extractFirstEmoji() trims the displayed value to one grapheme cluster."),
            ...
            HelpCard("Find alternative icons", "Bottom button — opens the model picker. Pick a set of (provider, model) pairs, tap Find Icons; each runs the bundled icons/translation_alt prompt (a variant of icons/translation that adds an explicit 'do not use a country flag emoji' instruction so the candidates aren't all flags). Results land on the Alternative icons live list. Tap a returned emoji to commit it as the new displayed value — the cache entry is overwritten with that call's provider/model/prompt/response."),
