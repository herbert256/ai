...
internal val HELP_TOPICS: Map<String, HelpContent> = mapOf(
    ...
    "agent_icon_detail" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Failure path", "When the icon call fails, the row keeps its ✅ — the agent itself succeeded, only the secondary icon call didn't. The failure reason lives here on the per-agent detail screen; re-running Create → Report icons retries.")
        )
    ),
