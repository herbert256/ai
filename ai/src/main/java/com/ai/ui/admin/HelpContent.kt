...
internal val HELP_TOPICS: Map<String, HelpContent> = mapOf(
    ...
    "icon_lookup" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Find alternative icons", "Bottom button — opens the model picker. Pick a set of (provider, model) pairs, tap Find Icons; each one runs the bundled `<base>_alt` variant of the scope's prompt. The `_alt` prompt is composed at runtime as `base.text + \"\\n\\n\" + alt.text` so the alt template only needs the nudge ('Do not repeat 📝 / 💬 / ✅' for most flows; 'Do not use a country flag emoji' for language/translation). Results land on the Alternative icons live list — tap a returned emoji to commit it as the new displayed value."),
