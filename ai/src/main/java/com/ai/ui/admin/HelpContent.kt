...
internal val HELP_TOPICS: Map<String, HelpContent> = mapOf(
    ...
    "report_result_generation" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("App-wide background resume sweep", "Once on app start and then every 2 minutes the app runs the same per-report stale-runs resume across every report modified in the last 7 days — translation placeholders that never converged, fan-out pairs the previous session abandoned, single-Meta calls left in PENDING. You don't have to open each report individually for its dropped work to come back. Logs land under the BgResumeSweep tag. Reports older than 7 days are skipped (out of the active-work window); a kill+restart catches them on next open.")
        )
    ),
