...
internal val HELP_TOPICS: Map<String, HelpContent> = mapOf(
    ...
    "about" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Version + install time", "`Version <X.Y.Z>` is the `versionName` from the manifest (set in `ai/build.gradle`). `Installed <timestamp>` is `PackageInfo.lastUpdateTime` — the moment the user's most recent `adb install` (or Play Store update) wrote the APK to the device. Avoids the gradle `BUILD_TIMESTAMP` which is stamped at configuration time and gets reused across builds when the Gradle configuration cache is on, so it goes stale fast in dev workflows."),
