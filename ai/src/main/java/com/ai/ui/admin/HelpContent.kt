...
internal val HELP_TOPICS: Map<String, HelpContent> = mapOf(
    ...
    "report_result_generation" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Create → Report icons", "Runs a 3-tier fallback chain per successful agent row. Tier 1: continue the conversation as a chat (user → assistant → user) and ask the model to emoji-fy its own previous answer (internal/report_icon_chat). Tier 2: one-shot internal/report_icon template against the agent's own (provider, model) with @PROMPT@/@RESPONSE@ substitution. Tier 3: fixed-agent fallback (DeepSeek) running internal/report_icon_3th. The first tier that returns an extractable emoji wins; all three failing falls back to 📝. Costs from every call (failed earlier tiers included) accumulate on the row's cost cell, post to global Usage statistics with kind=\"icon\", and appear as their own rows in the export's per-call All tab. Re-running wipes prior per-agent icons + chain audit log before kicking off."),
    ...
    "agent_icon_detail" to HelpContent(
        title = "Agent icon",
        cards = listOf(
            HelpCard("Overview", "Detail view for one agent's per-row emoji, reached by tapping the leftmost icon cell on a row that Create → Report icons has populated. Three cards (Model / Prompt / Response) plus a Find alternative icons button at the bottom."),
