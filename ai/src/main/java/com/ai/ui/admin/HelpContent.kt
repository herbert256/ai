...
internal val HELP_TOPICS: Map<String, HelpContent> = mapOf(
    ...
    "report_result_generation" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Create → Report icons", "Fires the bundled internal/icon prompt once per successful agent row in this report, each call against the agent's OWN (provider, model) — and substitutes @PROMPT@ with that agent's responseBody (not the report's prompt). On success, the row's leftmost ✅ swaps to the returned emoji, which becomes its own tap target for a per-agent icon detail. The per-call cost folds into the row's existing cost cell. Re-running wipes prior per-agent icons before kicking off."),
    ...
    "agent_icon_detail" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Prompt", "The internal/icon prompt template with @PROMPT@ substituted by THIS agent's responseBody (not the report's prompt). That's why each row can end up with a different icon — every model is iconifying its own answer."),
