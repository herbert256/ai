...
,
    "settings_main" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Subject to title bar", "Compact-header mode. Detail screens normally show a fixed title in the top bar (\"Model Info\" / \"Trace detail\" / \"Knowledge base\" / …) plus a green page-subject line below it (the model name, KB name, target language, …). Turn this on to fold the subject into the title bar and drop the green line — saves a row of vertical space across ~16 screens; long subjects truncate."),
    ...
    "internal_prompts_hub" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Fan out/in prompts", "Forwards to a sub-hub holding the five fan-* category CRUDs (fan_out, fan_in, fan_in_i, fan_in_r, fan_in_m). The badge count is the sum across all five buckets."),
    ...
    "fan_in_out_prompts_hub" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Manage Fan in, model, Initiator prompts", "category=\"fan_in_i\". Per-model initiator template."),
            HelpCard("Manage Fan in, model, Responder prompts", "category=\"fan_in_r\". Per-model responder template."),
            HelpCard("Manage Fan in, model, Initiator & Responder prompts", "category=\"fan_in_m\". Combined initiator + responder template."),
            HelpCard("Tips", "All five fan-* categories share the FAN_CATEGORIES treatment in the editor — no agent dispatch, the agent slot is N/A. Names are unique within each category, not across — same name can exist in fan_out and fan_in_i without collision."),
    ...
    "import_export" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Export · Settings", "GeneralSettings (userName, defaultEmail, defaultTypePaths, tracingEnabled, modelNameLayout, showBackButton, subjectToTitleBar) as a JSON object. Excludes the three info-provider API keys — those round-trip through API Keys instead."),
