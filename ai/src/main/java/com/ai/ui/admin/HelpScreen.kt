...
,
    ...
    "internal_prompts_hub" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Fan out/in prompts", "Forwards to a sub-hub holding the five fan-* category CRUDs (fan_out, fan_in, fan-in-i, fan-in-r, fan-in-m). The badge count is the sum across all five buckets."),
    ...
    "fan_in_out_prompts_hub" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Manage Fan in, model, Initiator prompts", "category=\"fan-in-i\". Per-model initiator template."),
            HelpCard("Manage Fan in, model, Responder prompts", "category=\"fan-in-r\". Per-model responder template."),
            HelpCard("Manage Fan in, model, Initiator & Responder prompts", "category=\"fan-in-m\". Combined initiator + responder template."),
            HelpCard("Tips", "All five fan-* categories share the FAN_CATEGORIES treatment in the editor — no agent dispatch, the agent slot is N/A. Names are unique within each category, not across — same name can exist in fan_out and fan-in-i without collision."),
