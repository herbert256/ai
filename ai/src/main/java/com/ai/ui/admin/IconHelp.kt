...
internal val DEFAULT_BAR_ICON_HELP: Map<String, Pair<String, String>> = mapOf(
    ...
    "👷" to ("Worker configuration" to "Open Report - setup: choose the report's system prompt and parameters, who generates the report info (icon, titles, language), the per-model info (icons & titles), and which pool the worker batches (Meta / Fan-in / Moderation / Tournament / Judges / Translation / Fan Meta) draw from — including Report models with When-available or Round-robin selection. Rerank and Compare always use their own prompt's workers."),
