...
internal val SCREEN_ICON_HELP: Map<String, List<Triple<String, String, String>>> = mapOf(
    ...
    "report_run" to listOf(
        ...
        Triple("👷", "Select workers", "Re-open Report - select workers to change who handles report info, model info and the worker batches."),
...
internal val DEFAULT_BAR_ICON_HELP: Map<String, Pair<String, String>> = mapOf(
    ...
    "👷" to ("Worker configuration" to "Open Report - select workers: choose who generates the report info (icon, titles, language), the per-model info (icons & titles), and which pool the worker batches (Meta / Fan-in / Rerank / Moderation / Tournament / Judges / Compare / Translation / Fan Meta) draw from — including Report models with When-available or Round-robin selection."),
