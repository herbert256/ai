...
internal val modelsHelp: Map<String, HelpContent> = mapOf(
    "model_statistics" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Hard-coded family rules", "Applied after the generic pass. (1) Bedrock vendor namespaces (mistral.ministral, nvidia.nemotron, openai.gpt-oss, amazon.nova, zai.glm) are stripped to the model — mistral.ministral → ministral, but deepseek.r1 → deepseek when the rest is just a version. (2) Prefix list (wan, command, flux, gemini, claude, gpt, grok, sonar, qwen, glm): anything starting with it collapses to that word — claude-haiku-4-5 → claude, gpt-oss-120b → gpt, every qwen3 / qwen2.5 / qwen-image → qwen. (3) Segment list (glm, aya, nemotron): an id with that as a -/./_ segment collapses to it — zai-glm → glm, nvidia-nemotron → nemotron — while autoglm-phone is left alone. (4) OpenAI o1 / o3 / o4 → o-series. Edit VENDOR_NAMESPACES / PREFIX_RULES / SEGMENT_RULES in ModelStatisticsScreen.kt."),
            HelpCard("Drill down", "Tap the Model name → every concrete provider · model that folds into it (tap a row → Model Info). Tap the Versions count → the distinct versions, each tappable to the providers offering it. Tap the Providers count → the distinct providers, each tappable to the versions it carries. Those leaf rows open Model Info. The Back gesture unwinds one drill level at a time."),
            HelpCard("Sorting", "Alphabetical by base model name (lower-cased, so casing folds together)."),
