...
internal val modelsHelp: Map<String, HelpContent> = mapOf(
    "model_statistics" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Hard-coded family rules", "Applied after the generic pass to fold families it can't. Prefix list (wan, command, flux, gemini, claude, gpt, grok, sonar): anything starting with it collapses to just that word — claude-haiku-4-5 → claude, gpt-oss-120b → gpt, command-r7b → command. Segment list (glm, aya): an id with that as a -/./_ segment collapses to it — zai.glm / zai-glm → glm, tiny-aya-earth → aya — while autoglm-phone is left alone. Edit PREFIX_RULES / SEGMENT_RULES in ModelStatisticsScreen.kt to condense further."),
