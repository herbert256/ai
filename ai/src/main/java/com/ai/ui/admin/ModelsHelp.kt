...
internal val modelsHelp: Map<String, HelpContent> = mapOf(
    "model_statistics" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Drill down", "Tap the Model name → every concrete provider · model that folds into it (tap a row → Model Info). Tap the Versions count → the distinct versions, each tappable to the providers offering it; when a family has more than 25 versions an extra grouping screen comes first (gpt → 5.2, 5.5, 4o, oss, …), tap a group to see its versions. Tap the Providers count → the distinct providers, each tappable to the versions it carries. Leaf rows open Model Info. Back unwinds one drill level at a time."),
