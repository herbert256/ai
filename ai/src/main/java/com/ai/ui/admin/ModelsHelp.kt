...
internal val modelsHelp: Map<String, HelpContent> = mapOf(
    "model_statistics" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("How the base name is found", "The namespace before the last / is dropped and the name lower-cased, then split on - _ : and space. The first token is kept as the name root; following tokens are appended until the first version/size/variant token — one that carries a digit (5.2, k2.6, 70b, v4, r1, 2024) or is a known variant word (instruct, chat, coder, vl, next, mini, thinking, …). So glm-5.2 → glm, kimi-k2.7-code → kimi, qwen3-coder → qwen3, llama-3.3-70b-instruct → llama, gpt-oss-120b → gpt-oss. A dot is not a separator so 5.2 stays one token. Model naming has no standard, so this is a best-effort heuristic."),
