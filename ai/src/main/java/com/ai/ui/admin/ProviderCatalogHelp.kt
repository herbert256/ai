...
internal val providerCatalogHelp: Map<String, HelpContent> = mapOf(
    ...
    "provider_edit" to HelpContent(
        ...
        cards = listOf(
            HelpCard("Overview", "Two layers in one screen: per-config runtime state at the top (key, Models row, Default Model row, plus Parameters / System-prompt icons in the bottom bar) and the catalog Definition cards underneath (base URL, admin URL, API format, type paths, pricing/feature flags — the id itself is fixed, there's no separate display name). All edits autosave."),
            ...
            HelpCard("API Key card", "One card, top to bottom: the key field, a Models row (drills into the per-provider Models screen — same one from AI Setup → Models → this provider), a Default Model row (opens the SelectModelScreen overlay — auto-refresh fetches the model list when the provider's source is API, and returning here updates the agent named after the provider in lock step with the chosen model), then Test. On pass: state flips to \"ok\" AND the provider's default agent gets added to the \"default agents\" flock. On fail: 🐞 inline icon links to the trace from this run when tracing is on."),
            HelpCard("Parameters & System prompt", "Not inline cards — both are icons in the bottom icon bar. Parameters opens the shared multi-select Parameters screen; System prompt opens the shared single-select System-prompt screen. Either selection autosaves back into this provider's config."),
            HelpCard("Definition cards", "Basics / API / Models / Pricing & cost / Throttle & retry overrides / Features / Native APIs / Capability flags / Model patterns / Built-in endpoints. Edits push through ProviderRegistry.update — same store loaded from the bundled `assets/providers/` catalog (one JSON file per provider) on first launch. ID and prefs key are read-only (changing them would orphan stored keys, models, statistics)."),
    ...
    "provider_anthropic" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Models", "Hero family: Claude 4 Opus / Sonnet (May 2025 ids in catalog, e.g. `claude-sonnet-4-20250514`, `claude-opus-4-20250514`), plus Claude 3.7 / 3.5 Sonnet, 3.5 Haiku, and the older 3 Opus / Sonnet / Haiku. Default model in the app is `claude-haiku-4-5-20251001` (Haiku 4.5). 8 hardcoded fallback models cover the major ids; live list comes from `v1/models`. `modelFilter=claude` keeps the picker tidy."),
    ...
    "provider_perplexity" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Pitfalls", "Citations only render when the chosen model supports them — a non-Sonar passthrough returns no citations, even though the response object still has the field. Deep-research calls can run for minutes; the app's default read timeout (240s, user-tunable in Settings) plus a 900s hard per-call ceiling usually cover it, but some upstream proxies don't. Search-recency filter is a string; typos silently disable filtering."),
        )
    ),
    "provider_together" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Models", "`Qwen/Qwen2.5-7B-Instruct-Turbo` (default), Llama 3.x / 4 Scout / Maverick Instruct Turbo, DeepSeek-V3 / R1, Qwen 3, Mixtral 8x22B, Mistral Small/Large, Gemma, Stable Diffusion. `modelFilter=chat|instruct|llama` keeps the catalog focused on chat-capable ids. `defaultModelSource=API`."),
    ...
    "provider_openrouter" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Models", "Default in the app: `ibm-granite/granite-4.0-h-micro`. Catalog spans every major provider — model ids are slash-prefixed (`anthropic/claude-3-5-sonnet`, `meta-llama/llama-3.3-70b-instruct`, `google/gemini-2.0-flash`). `defaultModelSource=API` — picker reads the live `/v1/models` list, which is large (hundreds of ids)."),
    ...
    "provider_mergegateway" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Models", "Default in the app: `ai21/jamba-1-5-large`. Catalog spans every routed provider — model ids are slash-prefixed. Note Anthropic ids use **dashes** for the version (`anthropic/claude-opus-4-8`, `anthropic/claude-sonnet-4-6`), while Google uses dots (`google/gemini-2.5-pro`, `google/gemini-2.5-flash`) and DeepSeek too (`deepseek/deepseek-v3.2`). `defaultModelSource=API` reads the live `v1/openai/models` list (200+ ids); `mergeHardcodedModels=true` keeps a small bundled Claude/Gemini/DeepSeek fallback visible before the first fetch. (The account's catalog may expose no `openai/*` ids — OpenAI models depend on Merge enabling them.)"),
    ...
    "provider_glama" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Models", "Default `zai/glm-4.6` (Z.AI's GLM). Slash-prefixed ids; Anthropic uses **dashes** (`anthropic/claude-opus-4-6`, `anthropic/claude-sonnet-4-6`) and OpenAI ids carry a date suffix (`openai/gpt-5.4-2026-03-05`). `defaultModelSource=API`; `mergeHardcodedModels=true` Claude/GPT/DeepSeek fallback (incl. `deepseek/deepseek-chat-v3`)."),
    ...
    "provider_requesty" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Models", "Default `openai-responses/gpt-4.1-nano` — the `openai-responses/` prefix explicitly routes through OpenAI's Responses API rather than Chat Completions, a third id convention alongside the dash/dot forms other gateways use. Slash-prefixed ids, Anthropic in **dash** form. Some ids carry routing suffixes like `:priority` / `:flex`. `defaultModelSource=API`; `mergeHardcodedModels=true` Claude/GPT/Gemini/DeepSeek fallback (incl. `google/gemini-2.5-flash-lite`)."),
    ...
    "provider_atlascloud" to HelpContent(
        ...
        cards = listOf(
            HelpCard("Overview", "Atlas Cloud (atlascloud.ai) — an OpenAI-compatible aggregator serving ~130 open-weight models (DeepSeek, Qwen, Llama, Gemini, GPT, ByteDance's Doubao, and Z.AI's GLM incl. `zai-org/glm-5.2`) behind one key."),
            ...
            HelpCard("Models", "Default `bytedance/doubao-seed-2.0-pro-260215`. Slash-prefixed ids; Z.AI casing is inconsistent (`zai-org/GLM-4.6` vs `zai-org/glm-5.2`) so use the live list (`defaultModelSource=API`). `mergeHardcodedModels=true` DeepSeek/GLM/Qwen/Gemini/GPT fallback (incl. `deepseek-ai/DeepSeek-V3-0324`)."),
    ...
    "provider_baseten" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Models", "Default `moonshotai/Kimi-K2.5`. Slash-prefixed HF-style ids — the hardcoded fallback list is `openai/gpt-oss-120b`, `zai-org/GLM-5.2`, `moonshotai/Kimi-K2.6`, `deepseek-ai/DeepSeek-V4-Pro`, `nvidia/Nemotron-120B-A12B`. `defaultModelSource=API` (the curated list is small); `mergeHardcodedModels=true` fallback. The catalog is the Model-APIs library, not Baseten's full deploy-your-own model universe."),
    ...
    "provider_siliconflow" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Models", "9 hardcoded fallback models — all text chat: `Qwen/Qwen2.5-7B/14B/32B/72B-Instruct`, `Qwen/QwQ-32B`, `deepseek-ai/DeepSeek-V3` / `DeepSeek-R1`, `THUDM/glm-4-9b-chat`, `meta-llama/Llama-3.3-70B-Instruct` (no image / embedding ids in the fallback list). Default in the app is `ByteDance-Seed/Seed-OSS-36B-Instruct`. Live list at `/v1/models` — `defaultModelSource=API` so the picker auto-refreshes."),
            HelpCard("Pricing & quirks", "OpenAI-compatible at the wire level. No native pricing tier — the layered lookup falls through to LiteLLM / models.dev / Helicone for whichever model id matches. No `litellmPrefix` set, so LiteLLM only matches when the model author's bare id is in their catalog. `nativeRerankUrl=v1/rerank` — SiliconFlow also wires the same generic `callRerankApi` native-rerank path Cohere uses."),
    ...
    "provider_cohere" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Models", "Default model in the app is `command-r7b-12-2024`. No hardcoded fallback list — the live model list drives the picker. Plus rerank models (`rerank-v3.5`, `rerank-multilingual-v3`) and the Embed family (`embed-english-v3`, `embed-multilingual-v3`)."),
            HelpCard("Pricing & quirks", "OpenAI-compatible chat at `compatibility/` base URL. **Native rerank endpoint wired** — `callRerankApi` in `RerankModerationApi.kt` routes any provider that declares `nativeRerankUrl` (Cohere's is `/v2/rerank`) and converts the response into the same `[{id, rank, score, reason}, ...]` shape the chat-model rerank flow uses; SiliconFlow uses the same generic path. `openRouterName=cohere`."),
    ...
    "provider_cerebras" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Models", "Default model in the app is `gpt-oss-120b`. No hardcoded fallback list — `defaultModelSource=API`, so `/v1/models` is the live source. Catalog is small by design — Cerebras only hosts a curated set that fits well on their wafer."),
    ...
    "provider_sambanova" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Models", "5 hardcoded fallback models: `Meta-Llama-3.3-70B-Instruct`, `DeepSeek-R1`, `DeepSeek-V3-0324`, `Qwen3-32B`, `Meta-Llama-3.1-8B-Instruct`. Default in the app is `gemma-4-31B-it`. Note the capitalised ids — SambaNova mirrors the upstream model author's casing exactly. Live list at `/v1/models`."),
    ...
    "provider_huggingface" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Models", "4 hardcoded fallback models: `meta-llama/Llama-3.3-70B-Instruct`, `deepseek-ai/DeepSeek-V3.1` (default), `Qwen/Qwen3-32B`, `openai/gpt-oss-120b`. Slash-prefixed ids match the HF Hub repo path. The Inference API supports far more models than the picker shows; copy a repo id from huggingface.co/models to use it."),
            HelpCard("Pricing & quirks", "OpenAI-compatible chat at `v1/chat/completions`. The base URL is `router.huggingface.co/` (HF's unified Inference Providers router, which replaced the older `api-inference.huggingface.co` host). Cold-start latency on rarely-used models is significant (HF spins down idle endpoints). Note: this is the HF *provider* (chat); the HF *info-provider* (model-card metadata) uses the same key but reads from `huggingface.co/api/models/{id}`."),
            HelpCard("Pitfalls", "Same `huggingFaceApiKey` is used by both this provider AND the model-card info-provider — they're conceptually distinct but share the key. Gated models (e.g. Meta's Llama family) return 401 / 403 until you accept terms on the HF Hub. HF Pro subscription is separate from API credit cost."),
        )
    ),
    "provider_deepinfra" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Models", "Default: `stepfun-ai/Step-3.7-Flash`. `chat` path is `chat/completions` (not `v1/chat/completions`); `modelsPath=models` (not `v1/models`). `defaultModelSource=API` so the picker auto-refreshes from the live list."),
    ...
    "provider_chutes" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Models", "Default: `google/gemma-4-31B-turbo-TEE` (a TEE-attested build — Trusted Execution Environment, verifiable output on untrusted decentralized miners). Slash-prefixed ids. Catalog: DeepSeek, Llama, Qwen, Mistral, Google's Gemma, plus Bittensor-native fine-tunes. `defaultModelSource=API` so picker auto-refreshes."),
