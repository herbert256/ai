...
internal val reportsHelp: Map<String, HelpContent> = mapOf(
    ...
    "report_select_workers" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Worker batches", "Which pool the batches draw from — Fan Meta, Translation, Tournament, Compare. (Meta + Fan-in have their own Meta card; Rerank, Moderation and Judge-the-judges are NOT listed: they run on the workers defined in their own prompt, whatever this card says.) Prompt configuration keeps each prompt's chain (a prompt set to *SELECT still asks at run time). Report models uses this report's own answer models. User selectable for each batch opens the worker picker on every batch start. One time selectable asks once — at the first batch — and reuses that group for every later batch."),
            HelpCard("Meta", "Worker pool for the Meta + Fan-in batches only, split out of Worker batches so they can run on a different pool. Identical options to Worker batches (Prompt configuration / Report models + Worker selection / User selectable for each batch / One time selectable)."),
            HelpCard("Worker selection", "Visible under Report models (on both the Worker batches and Meta cards). When available (the default) hands work to whichever model is free, so fast models absorb more items. Round robin deals items to the models in rotation so every model gets about the same share — a slow model keeps its items (they wait), but a model that errors (429, model gone) passes the item to the next in rotation."),
            HelpCard("Batches that ignore this card", "Some batches don't use the Worker batches choice at all. Rerank and Moderation run on the workers configured on their own Internal Prompt (a *SELECT prompt still asks at run time); Report models / SELECT modes do not apply. Judge-the-judges and Rank-the-translators have NO worker choice — each evaluates its own participants: Judge-the-judges re-judges with the actual judges that ran the report's Tournament (so it stays greyed until that Tournament finishes), and Rank-the-translators ranks with the actual translation models of the Translation batch it follows. Moderation also needs a worker whose provider has a native moderation endpoint (e.g. Mistral) — a chat model can't serve /moderations and the row errors."),
            HelpCard("Second result options", "Two switches (both off by default) that control the intermediate screens shown when you launch a Meta or Fan-out from Report - second results. Select scope shows the scope step first (which reports / languages the run covers); off skips it and uses all reports + all present languages. Runtime parameters shows the run-time editor first — edit the prompt for Meta, or pick the initiator / responder models and edit the prompt for Fan-out; off runs straight with the prompt as configured (with both off, Fan-out runs every successful model against every other). The other second results (Rerank, Moderation, Translate, Tournament, Judges, Compare, Rank-the-translators) have no such screens yet, so the switches don't affect them."),
            HelpCard("Editing later", "After generation, the 👷 icon on Report - manage reopens this screen without the Generate button (and without the System prompt / Parameters cards). Save persists the change for the report's future batches; the One-time group (once picked) is shown and editable there too.")
        )
    ),
