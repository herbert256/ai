...
internal val reportsHelp: Map<String, HelpContent> = mapOf(
    "value_view" to HelpContent(
        ...
        cards = listOf(
            HelpCard("What this is", "A cost × quality map of the models in this report. The horizontal axis is each model's cost; the vertical axis is its ranking score (quality). It answers \"which model gives most of the quality for the least cost?\". It appears once the report has a Rerank or a Tournament — that's where the quality scores come from."),
            HelpCard("Ranking switch", "The chips at the top pick which ranking feeds the quality axis: Rerank, or any of the Tournament methods (Copeland / Elo / Davidson / Tideman / Markov / Schulze / Minimax / Colley / Glicko-2 / Points / TrueSkill2). Tap one and the whole chart, the 💎 best-value pick and the list re-rank instantly — Tournament methods are recomputed locally from the stored win matrix, no new API calls."),
