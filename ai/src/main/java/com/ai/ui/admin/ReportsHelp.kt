...
internal val reportsHelp: Map<String, HelpContent> = mapOf(
    ...
    "value_view" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Tournament (total)", "Sits just before the individual methods. Each model's quality is the inverse of its AVERAGE position across all eleven Tournament methods — the same ordering as the Tournament screen's Total grid, so the best-on-average model sits highest."),
    ...
    "report_second_results" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Statistics line + top rows", "Under the title bar a statistics line shows the report's API-call count, total API time and running cost in cents (tap → the costs screen). The list leads with two link rows: report (the report's icon + title — tap to return to Manage) and info (the metadata-jobs aggregate — tap for Report - titles/icons/...)."),
    ...
    "report_select_models" to HelpContent(
        ...
        cards = listOf(
            HelpCard("Overview", "The model-selection page in the report flow. Empty model list at first; +Agent / +Flock / +Swarm / +Model / +Report fill it, Params lets you tweak the per-call parameter set, Next advances to Report - select workers, where Generate report fires the dispatch."),
            ...
            HelpCard("Next", "Advances to Report - select workers — the step that decides who generates the report info, the model info, and which pool the worker batches use. The Generate report button there fires the dispatch for every model in the list and flips the screen to Report - manage as soon as the first row starts streaming."),
    ...
    "report_select_workers" to HelpContent(
        title = "Help - Report — select workers",
        cards = listOf(
            HelpCard("Overview", "The third step of the report flow (prompt → models → workers). Three cards decide who does the worker jobs for this report; Generate report at the top fires the dispatch. Leave everything on Prompt configuration to keep the default behaviour — each worker prompt's configured chain."),
            ...
            HelpCard("Worker batches", "Which pool the batches draw from — Fan Meta, Translation, Tournament, Judge the judges, Compare, Rank the translators, and the single-call Rerank / Moderation / Meta / Fan-in. Prompt configuration keeps each prompt's chain (a prompt set to *SELECT still asks at run time). Report models uses this report's own answer models. User selectable for each batch opens the worker picker on every batch start. One time selectable asks once — at the first batch — and reuses that group for every later batch."),
            HelpCard("Worker selection", "Visible under Report models. When available (the default) hands work to whichever model is free, so fast models absorb more items. Round robin deals items to the models in rotation so every model gets about the same share — a slow model keeps its items (they wait), but a model that errors (429, model gone) passes the item to the next in rotation."),
            HelpCard("Rerank & Moderation note", "Both follow the Worker batches choice. Under Report models, Rerank runs each report model through the chat-JSON rerank path (quality may differ from a native rerank model), and Moderation only works when a report model's provider has a native moderation endpoint (Mistral) — otherwise the moderation row errors."),
            HelpCard("Editing later", "After generation, the 👷 icon on Report - manage reopens this screen without the Generate button. Save persists the change for the report's future batches; the One-time group (once picked) is shown and editable there too.")
        )
    ),
    "report_run" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Pick workers at run time", "Meta, Fan-in, Rerank, Moderation, Tournament, Judge the Judges, Translate, Compare and Rank the translators follow the report's Worker batches choice (👷 Report - select workers). Prompt configuration runs each batch on its Internal Prompt's workers — and a prompt set to *SELECT (Settings → Prompt management) opens the +Agent/+Flock/+Swarm/+Model picker first. User selectable for each batch forces that picker on every batch; One time selectable asks once and reuses the group; Report models never asks."),
            ...
            HelpCard("Reached from", "Pressing Generate report on Report - select workers. Or History → open any saved report — you land here directly, skipping the selection pages."),
