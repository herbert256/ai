...
internal val reportsHelp: Map<String, HelpContent> = mapOf(
    ...
    "report_export_sheet" to HelpContent(
        ...
        cards = listOf(
            HelpCard("Overview", "Pick a format, a detail level, and a target (where the export lands), then tap Export below the selection cards."),
            HelpCard("Export button", "Below the selection cards. Uses the selected Target — Android share / Browser / App. Disabled while a previous export is still building."),
            ...
            HelpCard("Target — Browser", "Builds the file and opens it as a separate Android intent (system browser for HTML; viewer app for PDF / Word / ODT). Stays on this screen so you can come back and try a different format without rebuilding picker state."),
            HelpCard("Target — App", "Renders the HTML inline in the in-app WebView preview — no external app launched. Only available when Format is HTML; the chip disappears for other formats and the selection auto-falls back to Browser."),
    ...
    "report_meta_run" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Parameters and system prompt", "Use the title-bar pickers to override these settings for this Meta run. Selected parameter presets apply in selection order; later values win. These choices override the internal prompt and worker defaults and are saved with the result for replay."),
    ...
    "report_runtime_prompt" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Temperature", "Tournament offers a temperature override for this run. Other operations use their prompt and worker parameter presets; this screen does not offer a temperature field for operations that cannot apply it."),
