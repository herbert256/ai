...
internal val reportsHelp: Map<String, HelpContent> = mapOf(
    ...
    "report_workers" to HelpContent(
        title = "Help - Workers",
        cards = listOf(
            HelpCard("Overview", "Who does the worker jobs for this report. Reached by tapping the Workers card on Report - setup (and, after generation, via the 👷 icon on Report - manage, which reopens Report - setup). Four collapsible cards, all collapsed on open, each carrying a one-line description of what it sets (expand to see and change the selection): Report info, Model info, Batches and Meta. Leave everything on its default to keep the standard behaviour."),
