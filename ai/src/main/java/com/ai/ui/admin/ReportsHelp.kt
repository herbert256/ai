...
internal val reportsHelp: Map<String, HelpContent> = mapOf(
    ...
    "report_select_workers" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Batches that ignore this card", "Rerank, Moderation and Judge-the-judges do not follow the Worker batches choice — they run on the workers configured on their own Internal Prompt (a *SELECT prompt still asks at run time); Report models / SELECT modes do not apply. Judge-the-judges is a type-A fixed-judge batch (each judge is the model being evaluated). Moderation also needs a worker whose provider has a native moderation endpoint (e.g. Mistral) — a chat model can't serve /moderations and the row errors."),
    ...
    "report_run" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Pick workers at run time", "Tournament, Compare, Translate and Rank the translators follow the report's Worker batches choice (👷 Report - setup); Meta and Fan-in follow that screen's separate Meta card. Prompt configuration runs each batch on its Internal Prompt's workers — and a prompt set to *SELECT (Settings → Prompt management) opens the +Agent/+Flock/+Swarm/+Model picker first. User selectable for each batch forces that picker on every batch; One time selectable asks once and reuses the group; Report models never asks. Rerank, Moderation and Judge-the-judges are the exceptions — they always use the workers defined in their own prompt, ignoring the Worker batches choice."),
