...
internal val reportsHelp: Map<String, HelpContent> = mapOf(
    ...
    "tournament_l1" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Statistics + grouping", "The counters show Total / Done / Error / Run / Wait / Queue / Costs (Wait = parked on a provider rate-limit cap). Matches are worker-judged, so a rate-limited judge is skipped and another picked — benched cells fold into Error rather than a separate column. The two chips switch how the matches below are grouped: 'Judge models' groups by the worker model that judged each match; 'Report models' groups by the report answer being compared. The green row fill shows progress for the active grouping. Tap a group to drill into its matches, then a match to see the two responses and the verdict."),
    ...
    "compare_l1" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Statistics + list", "The counters show Total / Done / Error / Run / Wait / Queue / Costs (Wait = parked on a provider rate-limit cap). Cells are worker-judged, so a rate-limited judge is skipped and another picked — benched cells fold into Error rather than a separate column. Below them each report answer is listed with its score against the meta result. Tap an answer to drill into its cell, then the cell for the full detail."),
