...
internal val reportsHelp: Map<String, HelpContent> = mapOf(
    ...
    "report_select_workers" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Worker batches", "Which pool the batches draw from — Fan Meta, Translation, Tournament, Judge the judges, Compare. (Meta + Fan-in have their own Meta card; Rerank and Moderation are NOT listed: they always run on the workers defined in their own prompt, whatever this card says.) Prompt configuration keeps each prompt's chain (a prompt set to *SELECT still asks at run time). Report models uses this report's own answer models. User selectable for each batch opens the worker picker on every batch start. One time selectable asks once — at the first batch — and reuses that group for every later batch."),
            ...
            HelpCard("Rerank & Moderation always use the prompt", "Unlike the other batches, Rerank and Moderation ignore the Worker batches choice entirely and run on the workers configured on their own Internal Prompt (a *SELECT prompt still asks at run time). Report models / SELECT modes do not apply to them. Note Moderation needs a worker whose provider has a native moderation endpoint (e.g. Mistral) — a chat model can't serve /moderations and the row errors."),
    ...
    "report_run" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Pick workers at run time", "Meta, Fan-in, Tournament, Judge the Judges, Compare, Translate and Rank the translators follow the report's Worker batches choice (👷 Report - setup). Prompt configuration runs each batch on its Internal Prompt's workers — and a prompt set to *SELECT (Settings → Prompt management) opens the +Agent/+Flock/+Swarm/+Model picker first. User selectable for each batch forces that picker on every batch; One time selectable asks once and reuses the group; Report models never asks. Rerank and Moderation are the exceptions — they always use the workers defined in their own prompt, ignoring the Worker batches choice."),
