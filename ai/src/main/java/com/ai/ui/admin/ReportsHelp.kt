...
internal val reportsHelp: Map<String, HelpContent> = mapOf(
    ...
    "report_select_workers" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Workers 👷", "Opens the full-screen Workers page — the four cards that decide who generates each worker job: Report info, Model info, Worker batches and Meta. (Split off this screen so the setup stays short.) Tap the card to open it; the ❓ on that page documents each card in detail."),
    ...
    "report_workers" to HelpContent(
        title = "Help - Workers",
        cards = listOf(
            HelpCard("Overview", "Who does the worker jobs for this report. Reached by tapping the Workers card on Report - setup (and, after generation, via the 👷 icon on Report - manage, which reopens Report - setup). Four collapsible cards, all collapsed on open, each showing its current selection: Report info, Model info, Worker batches and Meta. Leave everything on its default to keep the standard behaviour."),
