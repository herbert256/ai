...
internal val reportsHelp: Map<String, HelpContent> = mapOf(
    ...
    "report_select_workers" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Worker batches", "Which pool the batches draw from — Fan Meta, Translation, Tournament, Judge the judges, and the single-call Moderation / Meta / Fan-in. (Rerank and Compare are NOT listed: they always run on the workers defined in their own prompt, whatever this card says.) Prompt configuration keeps each prompt's chain (a prompt set to *SELECT still asks at run time). Report models uses this report's own answer models. User selectable for each batch opens the worker picker on every batch start. One time selectable asks once — at the first batch — and reuses that group for every later batch."),
            ...
            HelpCard("Rerank & Compare always use the prompt", "Unlike the other batches, Rerank and Compare ignore the Worker batches choice entirely and run on the workers configured on their own Internal Prompt (a *SELECT prompt still asks at run time). Report models / SELECT modes do not apply to them."),
            HelpCard("Moderation note", "Moderation follows the Worker batches choice. Under Report models it only works when a report model's provider has a native moderation endpoint (Mistral) — otherwise the moderation row errors."),
    ...
    "report_run" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Pick workers at run time", "Meta, Fan-in, Moderation, Tournament, Judge the Judges, Translate and Rank the translators follow the report's Worker batches choice (👷 Report - setup). Prompt configuration runs each batch on its Internal Prompt's workers — and a prompt set to *SELECT (Settings → Prompt management) opens the +Agent/+Flock/+Swarm/+Model picker first. User selectable for each batch forces that picker on every batch; One time selectable asks once and reuses the group; Report models never asks. Rerank and Compare are the exceptions — they always use the workers defined in their own prompt, ignoring the Worker batches choice."),
