...
internal val reportsHelp: Map<String, HelpContent> = mapOf(
    ...
    "report_run" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Pick workers at run time", "Meta, Fan-in, Rerank, Moderation, Tournament, Judge the Judges, Translate, Compare and Find-alternative normally run on the workers configured in their Internal Prompt. Set that prompt's Model selection to *SELECT (Settings → Prompt management) and the action instead opens the +Agent/+Flock/+Swarm/+Model picker first, so you choose the workers for this run."),
