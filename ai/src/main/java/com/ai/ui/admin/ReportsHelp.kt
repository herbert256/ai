...
internal val reportsHelp: Map<String, HelpContent> = mapOf(
    ...
    "icon_lookup_pair" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Cost attribution", "Bumped on the pair's `SecondaryResult.iconInputCost / iconOutputCost` (visible in the Cost line here and in the L2/L3 row totals). On Report → Manage the cost rolls up into the fan-out summary row's 🏷️ Fan Meta sibling. Per-call audit rows are `icon_fan_out` / `icon_fan_out_alt`, all attributed to the pair's SR."),
