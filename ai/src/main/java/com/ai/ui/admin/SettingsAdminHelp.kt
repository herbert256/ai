...
,
    ...
    "ai_live_dashboard" to HelpContent(
        ...
        cards = listOf(
            HelpCard("Overview", "A live ops monitor — what's happening right now. Opened from the home 📡 AI Live Dashboard card. For performance, only the first card (Live activity) is live on load; every other card starts collapsed — tap a header to expand it, and only then does it fetch and ~0.75 s-refresh its data. Collapse it again and it stops. The screen reopens all-collapsed each time. Lifetime totals + costs live on the Statistics hub (under Monitor)."),
            ...
            HelpCard("🧠 Local runtime", "On-device model activity: which local LLM / embedder models are held in memory and which one (if any) is generating / embedding right now. \"No on-device model loaded\" when idle."),
