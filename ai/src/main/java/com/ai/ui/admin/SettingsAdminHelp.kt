...
,
    ...
    "settings_logging" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Enable logging & tracing (master)", "Top-of-page master switch, default OFF — a fresh install records nothing. While off, all four settings below are hidden and forced off at runtime: no API tracing, no audit log, no usage statistics, and the file logger is disabled, whatever their individual values. Turn it on to record diagnostics and configure each item; the per-item values you set are preserved while the master is off, so flipping it back on restores them."),
