...
,
    ...
    "ai_live_dashboard" to HelpContent(
        ...
        cards = listOf(
            HelpCard("Overview", "A live ops monitor — what's happening right now. Auto-refreshes ~every 0.75 s while it's on top and stops the moment you leave it. Opened from the home 📡 AI Live Dashboard card. Lifetime totals + costs live on the Statistics hub (under Monitor)."),
            ...
            HelpCard("🌐 Provider throttle", "One row per host with an active gate, busiest first, host trimmed to its last two labels (generativelanguage.googleapis.com → googleapis.com): 'con' = concurrency in-use/limit, 'min' = calls in the trailing 60 s window vs the per-minute cap. Red when no concurrency slot is free. \"Idle — no active hosts\" when nothing is running. Mirrors ProviderThrottle.diagnostics() from the stall watchdog."),
            HelpCard("📊 HTTP responses", "A rolling tally of every network response code over the last 1 minute and last 5 minutes — ✅ 2xx, 🚧 429, ⚠️ 4xx, 🔥 5xx, and ▫️ other (network failures, 1xx/3xx). Counts each attempt, so the per-attempt 429s the retry logic later swallows still show here. In-memory only; resets on app restart."),
