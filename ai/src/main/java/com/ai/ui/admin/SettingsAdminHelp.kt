...
,
    ...
    "settings_logging" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Enable logging & tracing (master)", "Top-of-page master switch, default ON — diagnostics are recorded out of the box. While off, all four settings below are hidden and forced off at runtime: no API tracing, no audit log, no usage statistics, and the file logger is disabled, whatever their individual values. Turn it off to silence everything at once; the per-item values you set are preserved while the master is off, so flipping it back on restores them."),
            HelpCard("API tracing", "Master switch for ApiTracer. Off -> no new trace files are written. On -> API requests and responses are captured under filesDir/trace/ when a call path supplies trace tags. (Only active while the page master switch is on.)"),
    ...
    "broken_work" to HelpContent(
        ...
        cards = listOf(
            HelpCard("Overview", "One card per batch that needs attention. A background scan checks every report from the last 7 days — plus the currently-open report, whatever its age — at app start and every 30 seconds; while anything needs attention the top-bar AI logo is replaced by a ⚠️ that opens this screen."),
            ...
            HelpCard("Restart / delete", "Every count line carries per-line icons: ↻ restart re-fires errored items or re-dispatches unfinished ones in place (finished items are kept — no re-spend), 🗑 delete drops the covered items after a confirm (no API calls), and the view icon opens the item list. Tapping the card itself opens the item's own screen — the batch's screen for the batch families, the secondary detail for a single Meta / Rerank / Moderation, the Model response screen for a single broken model. Fan Meta unfinished can't be deleted — the underlying answer stays."),
            HelpCard("Coverage", "Covers all seven batch families — Fan Out, Fan Meta, Tournament, Judge the judges, Compare, Translation, and Rank the translators — plus a stalled regenerate job, single Meta / Rerank / Moderation calls, the report's own model responses, and failed title/icon/language metadata jobs (restart / delete; a single-item entry shows its error message right on the card; regenerate and the metadata card have no item list, so no view). A card drops off once its work is cleared, on the next scan."),
            HelpCard("Report models", "A \"Report models\" card covers the report's own model responses (not a secondary batch): models that errored, or were left PENDING/RUNNING by an app-kill (shown as interrupted). With one broken model the card shows the model's name and taps straight to its Model response screen; with several it shows \"Report models\" and opens a list of them. The 🗑 / ↻ on each count line delete or regenerate the covered models directly; the per-model list has the same icons per row."),
            HelpCard("Titles / icons", "A \"Titles / icons / ...\" card appears when report-metadata jobs failed — the report title, report icon, language, or a per-model title/icon (the red crosses on Report - titles/icons/...). It is restart-only: ↻ clears each errored job's stamp and re-runs it, exactly like that screen's own Restart errors. Tapping the card opens Report - titles/icons/... for the report."),
        )
    ),
    "broken_items" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Report models", "For a \"Report models\" batch with several broken models, tap a row to open its Model response screen, or use the row's own icons: 🗑 removes that model from the report (confirmed first), ↻ regenerates just that model."),
        )
    ),
    ...
    "internal_prompt_edit" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Model selection", "On the prompts that drive Meta, Fan-in, Rerank, Moderation, Tournament, Judge the Judges, Translate, Compare and Find-alternative: *CONFIGURED (default) runs against the workers above; *SELECT shows the +Agent/+Flock/+Swarm/+Model picker each time, just before that work starts, so you choose the workers per run (pre-filled with the configured chain; never saved back)."),
            HelpCard("Alternative prompts", "The Alternative (alt) prompts use the same Model/Agent/Flock/Swarm worker editor. If you configure at least one resolvable worker, starting a Find-alternative lookup skips the model-selection screen and generates the candidates on those workers (a flock/swarm gives several candidates). Leave the list empty to pick the models at run time as before."),
    ...
    "refresh_all" to HelpContent(
        ...
        cards = listOf(
            ...
            HelpCard("Providers needing attention", "Model-test and model-list failures appear separately, with complete errors and a shortcut to provider settings. When discovery succeeds but the default fails, choose a current default model and test it. A model error alone does not prove the API key is invalid."),
    ...
    "ai_usage_type" to HelpContent(
        ...
        cards = listOf(
            HelpCard("Overview", "Every usage entry for one raw Trace/Costs category, opened by tapping a row on the Spend & usage Types tab."),
