...
internal data class HtmlReportData(
    ...
    /** The report's user notes — rendered as an appendix so annotations
     *  survive into the shareable formats (they used to exist only in
     *  the JSON bundle round-trip). (label, text) pairs; label = the
     *  note's headline (or first line) plus its target. */
    val userNotes: List<Pair<String, String>> = emptyList(),
