 ...
 *  opened. */
private fun buildMediumBlocks(data: com.ai.ui.helpers.HtmlReportData, short: Boolean): List<DocBlock> {
    val out = mutableListOf<DocBlock>()
    val titleText = data.title.ifBlank { "Untitled" }
    val titlePrefix = com.ai.ui.helpers.iconPrefixPlain(data.reportIcon)
    ...
    val languages = com.ai.ui.helpers.buildLanguageViews(data)
    ...
    languages.forEachIndexed { i, lv ->
        ...
        if (i > 0) {
            ...
            val langHeading = com.ai.ui.helpers.languageLabelOrIconPlain(
...
private fun appendLanguageContent(
    ...
    data: com.ai.ui.helpers.HtmlReportData,
    short: Boolean,
    isOriginal: Boolean,
    maxAnchor: Int
) {
    if (data.agents.isNotEmpty()) {
        ...
        for (a in data.agents) {
            ...
            val iconPrefix = com.ai.ui.helpers.iconPrefixPlain(a.icon)
...
private fun appendMetaByName(
    out: MutableList<DocBlock>, secondary: List<com.ai.ui.helpers.HtmlSecondaryData>,
    maxAnchor: Int
) {
    val byName = LinkedHashMap<String, MutableList<com.ai.ui.helpers.HtmlSecondaryData>>()
    ...
    byName.forEach { (name, items) ->
        val headingIcon = com.ai.ui.helpers.iconPrefixPlain(com.ai.ui.helpers.metaPromptIcon(name))
        out += DocBlock(DocBlockKind.HEADING, "$headingIcon$name", 2)
        for (s in items) {
            val rowIcon = com.ai.ui.helpers.iconPrefixPlain(s.icon)
...
private fun appendCosts(out: MutableList<DocBlock>, data: com.ai.ui.helpers.HtmlReportData) {
...
private fun appendTraces(out: MutableList<DocBlock>, traces: List<com.ai.ui.helpers.HtmlTraceData>) {
...
internal fun buildDocxBytes(context: Context, report: com.ai.data.Report, short: Boolean = false): ByteArray =
    buildDocxBytesFromData(com.ai.ui.helpers.buildHtmlReportData(context, report), short)
...
internal fun buildDocxBytesFromData(data: com.ai.ui.helpers.HtmlReportData, short: Boolean = false): ByteArray {
...
internal fun buildOdtBytes(context: Context, report: com.ai.data.Report, short: Boolean = false): ByteArray =
    buildOdtBytesFromData(com.ai.ui.helpers.buildHtmlReportData(context, report), short)

/** Renders the ODT bytes from a pre-built [HtmlReportData].
 *  Per-language exports pass a `buildLanguageViews(base)` slice
 *  in here so the translated agent + meta text reaches the
 *  document instead of the original. */
internal fun buildOdtBytesFromData(data: com.ai.ui.helpers.HtmlReportData, short: Boolean = false): ByteArray {
    val blocks = buildMediumBlocks(data, short)
...
internal fun shareReportAsDocxOrOdt(
     ...
     *  [com.ai.ui.helpers.buildLanguageViews]. Null means caller hasn't
     *  resolved one — we build the all-languages base here. */
    data: com.ai.ui.helpers.HtmlReportData? = null,
