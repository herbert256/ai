...
object ProviderRegistry {
    ...
    private fun loadFromAssets(context: Context) {
        try {
            ...
            val list = defs.map { it.toAppService() }
            providers.clear()
            providers.addAll(list)
            save()
    ...
    fun save() {
        val sp = prefs ?: return
        val defs = providers.map { ProviderDefinition.fromAppService(it) }
        val gson = createAppGson()
     ...
     */
    fun ensureProviders(services: List<AppService>) {
        synchronized(lock) {
            var changed = false
...
data class ProviderDefinition(
    ...
    companion object {
        fun fromAppService(s: AppService): ProviderDefinition = ProviderDefinition(
            id = s.id,
