        ...
        AlertDialog(
            ...
            text = {
                Row(verticalAlignment = androidx.compose.ui.Alignment.CenterVertically) {
                    ...
                    Text(progressText.ifEmpty { "Working..." }, color = AppColors.TextSecondary)
                }
            },
            confirmButton = { },
                            ...
                            launchProgressTask("Refresh All", "Provider state + model lists...") {
                                ...
                                supervisorScope {
                                    ...
                                    val stateJob = async {
                                        ...
                                        val toTest = mutableListOf<AppService>()
                                        for (service in com.ai.data.AppService.entries) {
                                    ...
                                    val providersToTest = AppService.entries.filter { aiSettings.getApiKey(it).isNotBlank() }
                                    ...
                                    if (genResults.count { it.second } > 0) {
                                        val defaultAgentIds = updatedAgents.filter { agent -> AppService.entries.any { it.displayName == agent.name } }.map { it.id }
            ...
            AlertDialog(
                    ...
                    TextButton(
                        onClick = {
                            ...
                            when (cleanupType) {
                                ...
                                "reports" -> {
                                    val reports = com.ai.data.ReportStorage.getAllReports(context)
                ...
                dismissButton = {
                    TextButton(
                        ...
                        Text("Cancel", color = AppColors.Blue)
                    }
                },
                containerColor = AppColors.SurfaceDark
            )
...
private fun exportModelCostsToCsv(
    ...
    for (pm in sortedModels) {
        val provider = com.ai.data.AppService.findById(pm.provider)
...
private fun importModelCostsFromCsv(context: android.content.Context, uri: android.net.Uri): Pair<Int, Int> {
    ...
    try {
        ...
        for (i in 1 until lines.size) {
            ...
            // Map provider name to AppService
