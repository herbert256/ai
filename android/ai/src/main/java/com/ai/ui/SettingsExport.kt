 ...
 */
fun exportAiConfigToFile(context: Context, aiSettings: Settings, huggingFaceApiKey: String = "", openRouterApiKey: String = "") {
    // Build providers map
    val providers = AppService.entries.associate { service ->
    ...
    val providerDefinitions = AppService.entries.map { com.ai.data.ProviderDefinition.fromAppService(it) }
    ...
    try {
        ...
        val hasKeys = AppService.entries.any { aiSettings.getApiKey(it).isNotBlank() }
        if (hasKeys) {
 ...
 */
fun exportApiKeysToClipboard(context: Context, aiSettings: Settings) {
    val keys = AppService.entries.mapNotNull { service ->
 ...
 */
fun exportApiKeysToFile(context: Context, aiSettings: Settings, huggingFaceApiKey: String = "", openRouterApiKey: String = "") {
    val keys = AppService.entries.mapNotNull { service ->
        val apiKey = aiSettings.getApiKey(service)
 ...
 * Returns a triple of (updated Settings, huggingFaceApiKey, openRouterApiKey) or null on failure.
 */
fun importApiKeysFromFile(context: Context, uri: Uri, currentSettings: Settings): Triple<Settings, String?, String?>? {
    return try {
        ...
        for (entry in export.keys) {
            ...
            val service = AppService.findById(entry.service)
            if (service != null) {
...
internal fun processImportedConfig(
    ...
    export.providerDefinitions?.let { defs ->
        ...
        for (def in defs) {
            ...
            if (existing == null) {
                ...
                try {
                    com.ai.data.ProviderRegistry.update(def.toAppService())
                } catch (_: Exception) {}
            }
        }
    ...
    val agents = export.agents.mapNotNull { agentExport ->
        val provider = AppService.findById(agentExport.provider)
        provider?.let {
            Agent(
                id = agentExport.id,
        ...
        try {
            Swarm(
                ...
                members = swarmExport.members.mapNotNull { memberExport ->
                    val provider = AppService.findById(memberExport.provider) ?: return@mapNotNull null
                    SwarmMember(provider = provider, model = memberExport.model)
                },
    ...
    export.providerEndpoints?.forEach { providerEndpoints ->
        val provider = try {
            AppService.findById(providerEndpoints.provider)
        } catch (e: Exception) {
 ...
 * Returns ConfigImportResult or null on error. Does not show toasts on error.
 */
fun importAiConfigFromAsset(context: Context, assetFileName: String, currentSettings: Settings): ConfigImportResult? {
    return try {
        ...
        if (export.version !in 11..21) {
            android.util.Log.w("SettingsExport", "Unsupported setup config version: ${export.version}")
            return null
        }
...
fun performStartClean(
    ...
    com.ai.data.ReportStorage.getAllReports(context).forEach { report ->
