...
class SettingsPreferences(private val prefs: SharedPreferences, private val filesDir: File? = null) {
    ...
    /**
     * Save models for a single provider (used during API fetch to avoid saving entire Settings).
     */
    ...
    private fun loadAgents(): List<Agent> {
        ...
        return try {
            val list: List<Agent>? = gson.fromJson(json, TypeTokens.listAgentType)
            // Filter out agents with unknown providers; coalesce Gson null defaults
            list?.filter { AppService.findById(it.provider.id) != null } ?: emptyList()
        } catch (e: Exception) {
            emptyList()
        }
    }
    ...
    private fun loadFlocks(): List<Flock> {
        val json = prefs.getString(KEY_AI_FLOCKS, null) ?: return emptyList()
        return try {
            val list: List<Flock>? = gson.fromJson(json, TypeTokens.listFlockType)
            list ?: emptyList()
        } catch (e: Exception) {
            emptyList()
        }
    ...
    private fun loadSwarms(): List<Swarm> {
        val json = prefs.getString(KEY_AI_SWARMS, null) ?: return emptyList()
        return try {
            val list: List<Swarm>? = gson.fromJson(json, TypeTokens.listSwarmType)
            list ?: emptyList()
        } catch (e: Exception) {
            emptyList()
        }
    ...
    private fun loadParameters(): List<Parameters> {
        val json = prefs.getString(KEY_AI_PARAMETERS, null) ?: return emptyList()
        return try {
            val list: List<Parameters>? = gson.fromJson(json, TypeTokens.listParametersType)
            list?.map { params ->
                params.copy(
    ...
    private fun loadSystemPrompts(): List<SystemPrompt> {
    ...
    private fun loadEndpoints(): Map<AppService, List<Endpoint>> {
        ...
        return try {
            // Stored as Map<String, List<Endpoint>> where key is provider name
            val rawMap: Map<String, List<Endpoint>>? = gson.fromJson(json, TypeTokens.mapEndpointsType)
            rawMap?.mapKeys { entry -> AppService.findById(entry.key) }
    ...
    private fun saveEndpoints(endpoints: Map<AppService, List<Endpoint>>) {
        // Convert to Map<String, List<Endpoint>> for storage
    ...
    companion object {
        ...
        // Per-provider settings keys are now computed from AppService.prefsKey
    ...
    fun loadUsageStats(): Map<String, UsageStats> {
        ...
        return try {
            val json = file.readText()
            val list: List<UsageStats>? = gson.fromJson(json, TypeTokens.listUsageStatsType)
    ...
    fun isModelListCacheValid(provider: com.ai.data.AppService): Boolean {
    ...
    fun updateModelListTimestamp(provider: com.ai.data.AppService) {
        val key = KEY_MODEL_LIST_TIMESTAMP_PREFIX + provider.id
        prefs.edit { putLong(key, System.currentTimeMillis()) }
    }

    fun updateModelListTimestamps(providers: List<com.ai.data.AppService>) {
    ...
    fun clearModelListsCache() {
        prefs.edit {
            com.ai.data.AppService.entries.forEach { provider ->
