...
suspend fun AnalysisRepository.testModel(
    ...
    try {
        val response = when (service.apiFormat) {
            ApiFormat.ANTHROPIC -> analyzeWithClaude(service, apiKey, AnalysisRepository.TEST_PROMPT, model)
            ApiFormat.GOOGLE -> analyzeWithGemini(service, apiKey, AnalysisRepository.TEST_PROMPT, model)
            ApiFormat.OPENAI_COMPATIBLE -> analyzeWithOpenAiCompatible(service, apiKey, AnalysisRepository.TEST_PROMPT, model)
        }
    ...
    try {
        ...
        val effectiveJson = try {
            ...
            if (jsonElement.isJsonObject) {
                ...
                createAppGson().toJson(jsonElement)
            } else jsonBody
            ...
            if (response.isSuccessful) {
                ...
                try {
                    ...
                    if (content != null) {
                        ...
                        AnalysisResponse(service, responseBody, null, httpHeaders = headers, httpStatusCode = statusCode)
                    }
                } catch (e: Exception) {
                ...
                AnalysisResponse(service, null, "API error: $statusCode - $errorBody", httpHeaders = headers, httpStatusCode = statusCode)
            }
        }
    } catch (e: Exception) {
        AnalysisResponse(service, null, "Error: ${e.message}")
    }
}
 ...
 */
suspend fun AnalysisRepository.fetchGeminiModels(service: AppService, apiKey: String): List<String> = withContext(Dispatchers.IO) {
    try {
 ...
 */
suspend fun AnalysisRepository.fetchModels(service: AppService, apiKey: String): List<String> = withContext(Dispatchers.IO) {
    when (service.apiFormat) {
...
suspend fun AnalysisRepository.fetchClaudeModels(service: AppService, apiKey: String): List<String> = withContext(Dispatchers.IO) {
    try {
 ...
 */
suspend fun AnalysisRepository.fetchModelsOpenAiCompatible(service: AppService, apiKey: String): List<String> = withContext(Dispatchers.IO) {
