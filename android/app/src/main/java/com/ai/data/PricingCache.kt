...
object PricingCache {
     ...
     */
    fun setManualPricing(context: Context, provider: AppService, model: String, promptPrice: Double, completionPrice: Double) {
        synchronized(lock) {
            ensureLoaded(context)
     ...
     */
    fun removeManualPricing(context: Context, provider: AppService, model: String) {
        synchronized(lock) {
            ensureLoaded(context)
            val key = "${provider.id}:$model"
    ...
    private fun getOpenRouterPrefix(provider: AppService): String? {
        return provider.openRouterName
    }

    private fun getLiteLLMPrefix(provider: AppService): String? {
    ...
    /**
     * Map OpenRouter provider prefix to our AppService.
     * Reverse lookup of AppService.openRouterName.
     */
    private fun getAppServiceFromOpenRouterPrefix(prefix: String): AppService? {
        return AppService.entries.find { it.openRouterName == prefix }
    }
    ...
    suspend fun fetchAndSaveModelSpecifications(context: Context, apiKey: String): Pair<Int, Int>? {
        ...
        return try {
            ...
            for (model in models) {
                ...
                // Map OpenRouter prefix to our AppService
                val aiService = getAppServiceFromOpenRouterPrefix(openRouterPrefix)
     ...
     *
     * @param provider Our AppService enum
