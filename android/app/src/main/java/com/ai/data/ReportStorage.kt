...
object ReportStorage {
     ...
     * Get a report by ID
     */
    fun getReport(context: Context, reportId: String): Report? {
        init(context)
     ...
     */
    fun getReportsByDateRange(context: Context, startTime: Long, endTime: Long): List<Report> {
    ...
    private fun loadReport(reportId: String): Report? {
        ...
        return try {
            ...
            android.util.Log.e("ReportStorage", "Failed to load report $reportId: ${e.message}")
            null
        }
    }
    ...
    private fun loadAllReports(): List<Report> {
        ...
        return files.mapNotNull { file ->
            try {
                ...
                android.util.Log.e("ReportStorage", "Failed to load report from ${file.name}: ${e.message}")
                null
            }
        }
    }
    ...
    private fun saveReport(report: Report) {
        try {
            ...
            android.util.Log.e("ReportStorage", "Failed to save report ${report.id}: ${e.message}")
        }
    }
}
