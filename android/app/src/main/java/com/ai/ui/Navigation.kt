...
fun AppNavHost(
    ...
    LaunchedEffect(externalPrompt) {
        if (externalPrompt != null) {
            ...
            if (hasEdit) {
                ...
                viewModel.showGenericAgentSelection(externalTitle ?: "", fullPrompt)
    ...
    NavHost(
        ...
        composable(NavRoutes.AI_USAGE) {
            val uiState by viewModel.uiState.collectAsState()
            UsageScreen(
                openRouterApiKey = com.ai.data.AppService.findById("OPENROUTER")?.let { uiState.aiSettings.getApiKey(it) } ?: "",
                onBack = safePopBack,
                onNavigateHome = navigateHome
            )
        }
...
fun SettingsScreenNav(
    ...
    SettingsScreen(
        ...
        onSaveAi = { viewModel.updateSettings(it) },
        onTestAiModel = { service, apiKey, model -> viewModel.testAiModel(service, apiKey, model) },
