...
internal fun convertGenericReportsToHtml(context: android.content.Context, uiState: UiState, appVersion: String): String {
    val data = HtmlReportData(
        title = uiState.genericPromptTitle,
        prompt = uiState.genericPromptText,
        timestamp = java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss", java.util.Locale.getDefault())
            .format(java.util.Date()),
        rapportText = null,
        closeText = uiState.externalCloseHtml,
        developerMode = uiState.generalSettings.developerMode,
        agents = uiState.genericReportsAgentResults.entries.mapNotNull { (agentId, response) ->
...
internal fun shareReportAsJson(context: android.content.Context, reportId: String) {
    try {
        val report = com.ai.data.ReportStorage.getReport(context, reportId)
        if (report == null) {
            android.widget.Toast.makeText(context, "Report not found", android.widget.Toast.LENGTH_SHORT).show()
            return
        }

        val gson = com.ai.data.createAppGson(prettyPrint = true)
...
internal fun shareReportAsHtml(context: android.content.Context, reportId: String, developerMode: Boolean = false) {
    try {
        ...
        val html = convertReportToHtml(context, report, appVersion, developerMode)
...
internal fun openGenericReportsInChrome(context: android.content.Context, uiState: UiState) {
    try {
        val appVersion = try {
            context.packageManager.getPackageInfo(context.packageName, 0).versionName ?: "unknown"
        } catch (e: Exception) { "unknown" }
        val html = convertGenericReportsToHtml(context, uiState, appVersion)
 ...
 */
internal fun convertReportToHtml(context: android.content.Context, report: com.ai.data.Report, appVersion: String, developerMode: Boolean = false): String {
    val data = HtmlReportData(
        title = report.title,
        ...
        agents = report.agents.filter { it.reportStatus != com.ai.data.ReportStatus.STOPPED && it.reportStatus != com.ai.data.ReportStatus.PENDING }.map { agent ->
            val providerEnum = com.ai.data.AppService.findById(agent.provider)
...
private fun renderHtmlReport(data: HtmlReportData, appVersion: String): String {
    ...
    // Insert close text (from external intent <close> tag) before footer
    data.closeText?.takeIf { it.isNotBlank() }?.let { closeText ->
        htmlBuilder.append("""<hr style="border:none;border-top:2px solid #333;margin:20px 0 10px 0">""")
        htmlBuilder.append(closeText)
    }
