...
internal fun emailReportAsHtml(context: android.content.Context, reportId: String, emailAddress: String, developerMode: Boolean = false): Boolean {
    ...
    return try {
        ...
        val uri = FileProvider.getUriForFile(context, "${context.packageName}.fileprovider", file)
        val intent = Intent(Intent.ACTION_SEND).apply {
            type = "message/rfc822"; `package` = "com.google.android.gm"
            putExtra(Intent.EXTRA_EMAIL, arrayOf(emailAddress)); putExtra(Intent.EXTRA_SUBJECT, "AI Report - ${report.title}")
            putExtra(Intent.EXTRA_STREAM, uri); addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
        }
...
private fun convertReportToHtml(context: android.content.Context, report: Report, appVersion: String, developerMode: Boolean = false): String {
    val agents = report.agents
        ...
        .map { agent ->
            ...
            HtmlAgentData(
                ...
                rawUsageJson = null, responseHeaders = agent.responseHeaders,
    ...
    return if (data.reportType == ReportType.TABLE) renderTableHtmlReport(data, appVersion) else renderHtmlReport(data, appVersion)
}
...
private fun renderHtmlReport(data: HtmlReportData, appVersion: String): String {
    ...
    data.rapportText?.let { sb.append("<div class='rapport'>$it</div>") }
    ...
    // Prompt & costs sections
    appendPromptAndCosts(sb, data)
    data.closeText?.let { sb.append("<div class='close-text'>$it</div>") }
