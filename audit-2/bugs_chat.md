...
**Status:** Fixed — single chat now derives totalCost as `(inTok*promptPrice + outTok*completionPrice)*100`, identical formula/units to DualChatScreen (ChatScreens.kt ~395, ~540). promptPrice is per-token consistently. Full shared-helper extraction lives in data-layer Pricing (Bug 78).
...
**Status:** Fixed — assistantMsg + outputTokens now built from `sb.toString()` (finalContent) instead of streamingContentState.value (ChatScreens.kt ~536-545).
...
**Status:** Fixed — collapsed the three loadSession calls into one `remember(currentSessionId)` (persistedSession); pinned/KB/title now derive from it (ChatScreens.kt ~393-418).
...
**Status:** Fixed — saveSession builds the record on the caller thread then writes via `scope.launch(Dispatchers.IO)`; setSessionPinned toggle also dispatched to IO (ChatScreens.kt ~431, ~684).
...
**Status:** Fixed — send now wraps with `withTracerTags(reportId = currentSessionId, category = "Chat")`; ChatMessageBubble filters traces on sessionId (falls back to untagged) before the closest-timestamp tiebreak (ChatScreens.kt ~533, ~988).
...
**Status:** Won't fix — banner already resets at the top of trySend (existing). Recording moderation usage needs a provider/model/kind-aware stats callback; the only hook here is `onRecordStatistics(Int,Int)` which attributes to the chat model. The recording path lives in viewmodel/data — deferred to that pass.
...
**Status:** Fixed — key changed from positional `..._$idx` to non-positional `role_timestamp_contentHash` so insertions no longer re-key trailing items (ChatScreens.kt ~724). A true stable id on ChatMessage lives in data/ (DataModels.kt) — deferred; content-hash key is sufficient and ui-scoped.
...
**Status:** Fixed — added `rememberSaveable starterConsumed`; onConsumeStarter fires once and userInput only seeds from starter when not yet consumed (ChatScreens.kt ~294-306).
...
**Status:** Won't fix — acknowledged LOW limitation; the cache-file-path Saver adds file lifecycle/cleanup complexity for a narrow not-yet-sent-image process-death window. Out of proportion for the severity.
...
**Status:** Fixed — LaunchedEffect now keys on `targetLineCount` (lines.size) instead of `content`, so the reveal loop walks monotonically to the target without restarting per chunk or clamping down mid-stream (ChatScreens.kt ~1103-1120).
...
**Status:** Fixed — replaced the totalCost accumulator with cumulative totalInputTokens/totalOutputTokens; totalCost is now `derivedStateOf` over current `pricing`, so cold-window turns re-price once PricingCache primes (ChatScreens.kt ~310, ~398-405, ~540).
...
**Status:** Won't fix — fix lives in viewmodel/ChatViewModel.kt, deferred to that pass (out of scope for the ui/ chat pass).
...
**Status:** Won't fix — fix lives in viewmodel/ChatViewModel.kt, deferred to that pass.
...
**Status:** Won't fix — the cloud + local chat send path in ChatScreens calls onRecordStatistics uniformly (no LOCAL short-circuit in the ui send path); any remaining gap in how the stat is recorded lives in viewmodel/data — deferred to that pass.
...
**Status:** Won't fix — `messagesWithRag` lives in viewmodel/ChatViewModel.kt, deferred to that pass.
...
**Status:** Fixed — moved `withTracerTags(reportId = sessionId, category = "Dual chat")` from around the whole loop to wrap each individual sendDualChatMessage call (DualChatScreen.kt ~440, ~455).
...
**Status:** Fixed — startChatLoop now early-returns if isRunning, cancels the old chatJob, and flips isRunning synchronously on the main thread before launching (DualChatScreen.kt ~426-432).
...
**Status:** Fixed — introduced `DUAL_MSG_STRIDE` constant and changed the restore loop to `while (i + STRIDE <= flat.size)` (DualChatScreen.kt ~69-101).
...
**Status:** Fixed — added a UUID `id` to DualMessage (persisted via the Saver) and keyed the LazyColumn on it (DualChatScreen.kt ~57, ~498).
...
**Status:** Fixed (convention) — single chat now uses the same `(inTok*p + outTok*c)*100` derivedStateOf formula as dual chat (Bug 1), so the underlying cost math no longer diverges. Extracting one shared formatter lives in data-layer Pricing (Bug 78).
...
**Status:** Fixed — single chat (Bug 11) now uses the same "cumulative tokens × latest price via derivedStateOf" convention as dual chat, so both screens are consistent.
...
**Status:** Won't fix — DualChatConfig holds AppService objects (not trivially Parcelable) and the proper fix is to persist the config in a session record (data layer). The screen already degrades gracefully (navigates back). Deferred to a data-layer session-record change.
...
**Status:** Won't fix — provider-format role normalization belongs in the dispatch/data layer (ApiDispatch/ApiStreaming), not the dual-chat screen. Deferred to that pass.
...
**Status:** Fixed — removed the inline `animateScrollToItem(messages.size-1)` calls inside the loop and added a `LaunchedEffect(messages.size)` auto-scroll like single chat (DualChatScreen.kt ~471-476).
...
**Status:** Fixed — folded the two param-id JSON list writes into the single savePrefs editor commit; removed the now-unused saveStringList helper (DualChatScreen.kt ~172-187).
...
**Status:** Fixed — added `MAX_DUAL_ROUNDS = 100`; the config interactionCount is coerced to 1..100 and the "Chat N more" extra count is coerced to ≤100 (DualChatScreen.kt ~57, ~291, ~581).
...
**Status:** Won't fix — an explicit interruption flag belongs on the ChatSession record (data layer); inferring it differently in the hub can't distinguish the cases. Deferred to a data-layer session-record change.
...
**Status:** Fixed — onClick now uses `installed.singleOrNull()`: a lone model opens directly, any other count (incl. empty) opens the dropdown rather than blindly indexing first() (ChatHub.kt ~189).
...
**Status:** Fixed — settled on the explicit model: searching only runs on a Search tap (hasSearched trigger), and any edit to the query resets hasSearched so a new query needs a fresh tap. Removed the per-keystroke debounce effect (ChatHistory.kt ~200-214, ~223).
...
**Status:** Fixed — switched to itemsIndexed with key `sessionId:timestamp:role:index` so same-session same-timestamp matches no longer collide (ChatHistory.kt ~256).
...
**Status:** Fixed — matchIndex/start/end are now all coerceIn against `message.content.length` (end coerced against start..length) so case-folding length divergence can't produce an out-of-bounds substring (ChatHistory.kt ~161-166).
...
**Status:** Won't fix — the paged-with-swipe UX is intentional and subList bounds are already guarded (no crash). A measured-layout rewrite is disproportionate for a LOW dp-estimate fragility.
...
**Status:** Fixed — `cutoff` is now `remember(daysText)` (frozen per open/daysText) and is the produceState key, so the displayed count and the delete set derive from the same snapshot (ChatManageScreen.kt ~127-136).
...
**Status:** Fixed — zipAllChats now counts the entries actually written (and only .json files) rather than the parsed-session count (ChatManageScreen.kt ~170-185).
...
**Status:** Fixed — historyDir.listFiles now filters to `f.extension == "json"` before zipping (ChatManageScreen.kt ~177).
...
**Status:** Won't fix — fix lives in data/ChatHistoryManager.kt, deferred to the data-layer pass.
...
**Status:** Fixed (partial) — added an `ingestedBatchKey` guard so a new-but-equal pendingUris instance can't double-ingest before onConsumePending clears it (KnowledgeScreens.kt ~296-302). The progress-via-scope.launch part is Won't fix here: IndexProgress is a non-suspend data-layer fun interface so withContext can't be called inside it; the screen-scope launches are already cancelled on disposal (see Bug 79).
...
**Status:** Fixed — the Delete-source handler now runs `KnowledgeStore.deleteSource` inside `scope.launch { withContext(Dispatchers.IO) { ... } }` then bumps refreshTick (KnowledgeScreens.kt ~465).
...
**Status:** Fixed — removed the unconditional `LaunchedEffect(Unit){refreshTick++}`; resumeTick (ON_RESUME) already refreshes the KB list on entry (KnowledgeScreens.kt ~130).
...
**Status:** Fixed — localEmbedders now keyed on `resumeRefreshTick()` and `selected` is revalidated against the option set in a LaunchedEffect (KnowledgeScreens.kt ~166-180).
...
**Status:** Fixed — Apply now recomputes the anchor embedder from the current knowledgeBases and drops any selected id whose embedder no longer matches before calling onConfirm (KnowledgeScreens.kt ~644-658).
...
**Status:** Won't fix — pickTypeForUri is non-nullable and called from multiple ingest paths; making it reject requires a nullable contract plus user-error handling at every call site. For a LOW issue (binary content extracts to noise/empty and is bounded) the ripple isn't justified.
...
**Status:** Fixed — the share-target auto-ingest fallback name now appends the batch index (`shared_<ms>_<idx>`) so same-millisecond burst files don't collide (KnowledgeScreens.kt ~305, ~323). The single-file pickFile path picks one file at a time, so its fallback is collision-safe as-is.
...
**Status:** Fixed — usageEntry now uses the same 3-part prefix aggregate (`"${provider.id}::$modelName::"`) as the View screen, summing across kinds (ModelScreens.kt ~224-242).
...
**Status:** Fixed — resolved automatically by Bug 45: usageEntry is now populated, so usageCost / hasUsageStats / the cumulative AI-Usage row render when stats exist.
...
**Status:** Fixed (TTL) — added a 6h TTL to both ModelInfoCache (ModelScreens.kt) and ModelInfoLookupCache (ModelInfoViewScreen.kt) so a stale catalog isn't served for the whole process. Merging into one shared cache left as a non-essential refactor.
...
**Status:** Fixed — Manage now reads the AI intro via `PromptCache.getRaw` (no destructive 48h TTL), matching the View screen's read path so a cached intro doesn't silently disappear after 48h (ModelScreens.kt ~406).
...
**Status:** Fixed — ModelUsageEntry gained a `navigable` flag (false for chat rows); the row's `clickable` is only applied when navigable, so chat rows no longer show a dead ripple (ModelScreens.kt ~40-45, ~69, ~1147).
...
**Status:** Fixed — chat candidates are now bounded to the newest 30 matching sessions, and the report/secondary cap counts only entries added since the chat phase (reportStartSize) so reports are always considered (ModelScreens.kt ~64-110).
...
**Status:** Fixed — for the LOCAL provider, providerHost is set to "local" explicitly (matching LocalLlm/LocalEmbedder trace hostnames) instead of null→name-only match (ModelScreens.kt ~215-221).
...
**Status:** Fixed — the HF fetch records the matched candidate id in `hfMatchedId`; the raw-source overlay's calledUrl now uses it (`/api/models/${hfMatchedId ?: modelName}`) (ModelScreens.kt ~343-368, ~693).
...
**Status:** Won't fix — Pricing has no per-kind / search-unit tier (only promptPrice/completionPrice), so per-kind pricing isn't expressible without data-layer Pricing support. The token-based aggregate is an intentional approximation; precise per-kind pricing is deferred to a data-layer change.
...
**Status:** Fixed — auto-fire now happens only when there is NO cached intro at all; a merely-stale (>1 week) intro is shown as-is and refreshed via the user-initiated "Ask again" affordance, stopping silent spend while browsing (ModelInfoViewScreen.kt ~266-274).
...
**Status:** Fixed — deep-link now uses `matchingAgent.agentId.ifBlank { matchingAgent.agentName }` (ReportAgent's only other identity field) (ModelInfoViewScreen.kt ~965).
...
**Status:** Fixed — ParsedJsonValue caps recursion at MAX_JSON_RENDER_DEPTH (12) with a tap-to-expand raw blob (horizontally scrollable), and clamps cumulative left indent (ModelInfoViewScreen.kt ~857-918).
...
**Status:** Fixed — `vecs[j]` replaced with `vecs.getOrNull(j) ?: continue` so a short embed response no longer throws IOOBE (SemanticSearchScreen.kt ~243).
...
**Status:** Won't fix — storing/checking embedding dim lives in data/EmbeddingsStore.kt (cache key + cosine). Deferred to the data-layer pass.
...
**Status:** Fixed — onProgress is now `suspend (String) -> Unit` and the callsite hops via `withContext(Dispatchers.Main)` instead of `scope.launch(Dispatchers.Main)`, so progress writes are lifecycle-bound (SemanticSearchScreen.kt ~143; same change in LocalSemanticSearchScreen.kt).
...
**Status:** Fixed — a LaunchedEffect(embeddingChoices) resets `picked` to the first valid choice when it's no longer in the list (SemanticSearchScreen.kt ~68-74).
...
**Status:** Fixed — `vecs[j]` replaced with `vecs.getOrNull(j) ?: continue` (LocalSemanticSearchScreen.kt ~222).
...
**Status:** Fixed — both screens now carry title + timestamp as separate fields on dedicated EmbedCandidate/CachedCandidate (Cloud*-prefixed in the cloud screen) data classes instead of packing them into one " — "-joined string that was split back (LocalSemanticSearchScreen.kt ~200-232; SemanticSearchScreen.kt ~223-263).
...
**Status:** Fixed — capture `searchStartedAt` when the search starts; the latestTrace produceState picks the category trace with `timestamp >= searchStartedAt` so the 🐞 opens this run's trace, not the prior one (LocalSemanticSearchScreen.kt ~63-72, ~130).
...
**Status:** Fixed — availableModels now keyed on `resumeRefreshTick()` and `picked` is revalidated when the model set changes (LocalSemanticSearchScreen.kt ~56-62).
...
**Status:** Fixed — QuickHit gained an `epoch: Long` field; sorting is now `sortedByDescending { it.epoch }` (QuickLocalSearchScreen.kt ~125-139).
...
**Status:** Won't fix — perf-only (LOW); the search already runs off the main thread, and memoizing the report list across these three screens is a non-essential optimization. ReportStorage caching is the better home for it (data layer).
...
**Status:** Fixed — LocalSearchHit gained `epoch: Long`; the `.thenByDescending` tiebreak now sorts on epoch (LocalSearchScreen.kt ~128-152).
...
**Status:** Fixed — scoring now uses word-boundary regexes (`\b<token>\b`) via findAll().count() instead of advancing-substring indexOf, so short tokens no longer match inside unrelated words (LocalSearchScreen.kt ~130-152).
...
**Status:** Fixed — paging is deferred with `if (maxHeight.value <= 0f) return@BoxWithConstraints` so the pre-measure frame doesn't compute pageSize=1 / flicker (HistoryScreen.kt ~71-74).
...
**Status:** Fixed — the delete handler now recomputes totalPages from the post-delete size and clamps currentPage in the same handler, before the reactive effect fires (HistoryScreen.kt ~140-146).
...
**Status:** Fixed — filteredReports is now a produceState with a 250ms debounce that runs the filter on Dispatchers.Default; the previous result stays visible while debouncing (HistoryScreen.kt ~61-74).
...
**Status:** Fixed — currentPage is now `rememberSaveable` (HistoryScreen.kt ~59).
...
**Status:** Fixed — loadPromptHistory now runs in a LaunchedEffect on Dispatchers.IO into a `loaded` state instead of synchronously in the initializer (PromptHistoryScreen.kt ~36-40).
...
**Status:** Fixed — clearPromptHistory now runs in `scope.launch(Dispatchers.IO)`; the UI clears immediately via an overrideEntries state (PromptHistoryScreen.kt ~62).
...
**Status:** Fixed — items key is now composite `timestamp:titleHash:promptHash` so same-millisecond entries don't collide (PromptHistoryScreen.kt ~90).
...
**Status:** Fixed — added an `else if (sorted.isEmpty())` branch rendering a "No prompts match …" empty state (ExamplePromptPickerScreen.kt ~65-72).
...
**Status:** Won't fix (per-screen fixes applied) — the individual correctness issues are fixed (Bug 5: chat now filters on sessionId; Bug 63: local search filters on callStart). Extracting one shared helper would touch ui/shared (out of scope) and risks the documented overlay/trace conventions; the minimal-safe per-screen fixes are preferred.
...
**Status:** Won't fix (math aligned) — the cost-from-tokens math is now identical between single and dual chat (Bugs 1/20/21). A single `Pricing.costCents()` + shared formatter belongs on the data-layer Pricing class; deferred to that pass.
...
**Status:** Fixed (search screens) — SemanticSearchScreen + LocalSemanticSearchScreen now use a `suspend (String)` progress callback that hops via `withContext(Dispatchers.Main)`. The Knowledge screens (38/39/40/45 lines) still use `scope.launch(Dispatchers.Main)` because IndexProgress is a non-suspend data-layer fun interface; those launches are screen-scoped (cancelled on disposal), so left as-is — a true suspend IndexProgress is a data-layer change.
