...
**Status:** Fixed — assign testSuccess/testResult from the call result; AgentsScreen.kt:291
...
**Status:** Fixed — selectedAgentIds now a LinkedHashSet (order-preserving) and paramsIds .distinct() on save; FlocksScreen.kt:38,103
...
**Status:** Fixed — availableAgents now includes selected-but-inactive agents (count + uncheck), rows tagged "(inactive)"; FlocksScreen.kt:60,131
...
**Status:** Fixed — onSave now receives isAddMode; a non-dup edit that repoints prunes the original key first (MOVE not duplicate); costsmanualoverride/edit.kt:29, StatisticsScreen.kt onSave signature extended
...
**Status:** Fixed — buildGeneralSettingsTree/applyGeneralSettings now carry metadata sub-flags, metadataIcons, app-wide/report-model prompt+params, all network/throttle/retry caps, logLevel, recentReportModels; ImportExportScreen.kt:265,281 (API keys still deliberately excluded — they ride the API Keys export)
...
**Status:** Fixed — importType now rememberSaveable; ImportExportScreen.kt:709
...
**Status:** Fixed — numeric reads guarded with isNumber + runCatching and the whole costs loop wrapped in try/catch; ImportExportScreen.kt:~1422
...
**Status:** Won't fix — matches the established non-blocking RestartAppBanner pattern (same as restore); atomic apply or a blocking gate is a large refactor disproportionate to a MEDIUM with the documented mid-import tradeoff.
...
**Status:** Not a bug — ExamplePrompt is {id,title,text}; id is regenerated by the seed importer and title+text are the only content fields, so the bundle shape is lossless. No new fields to flag.
...
**Status:** Fixed — doc comment now says keyed by AppService.id; ApiKeyTransfer.kt:28
...
**Status:** Won't fix — fix lives in data/BackupManager.kt, out of scope; also guarded (validate-then-write restore ordering / preserve-set must not be restructured).
...
**Status:** Fixed — reworded confirm dialog + card copy to "prompted to restart"/"tap Restart when prompted"; BackupRestoreScreen.kt:80,145
...
**Status:** Fixed — failure toast now appends "— existing data left unchanged."; BackupRestoreScreen.kt:102
...
**Status:** Won't fix — proper fix is a full in-memory _uiState reload in the viewmodel (out of scope); the four-button post-reset banner is an intentional design per the inline comment.
...
**Status:** Fixed — distinct toast for the n==0 (read OK but empty) case vs n<0 (couldn't read); ResetScreen.kt:210
...
**Status:** Fixed — split into copyApkToCache (IO) + fireInstallIntent (main); button launches a coroutine with an isInstalling busy state; UpdateFromCloudScreen.kt:131,312
...
**Status:** Fixed — copyApkToCache writes update.apk.tmp then renameTo(update.apk); deletes the tmp on any failure; UpdateFromCloudScreen.kt:312
...
**Status:** Not a bug — intentional; the inline comment (TrimByAgeScreen.kt:40-43) documents staying on the UI thread for consistency with the synchronous Trim path. LOW jank only.
...
**Status:** Fixed — cutoff now remember(days)'d so count and delete share one instant; TrimByAgeScreen.kt:39
...
**Status:** Fixed — a one-column edit now keeps the other side at the current getPricing value instead of skipping; CostsMaintenanceScreen.kt:120
...
**Status:** Won't fix — LOW; would require threading a coroutine scope through all three SAF result callbacks; the per-model tier lookups are bounded by active providers × models.
...
**Status:** Fixed — dropped the items.size key on the page remember; safePage already coerces; CrudListPage.kt:59
...
**Status:** Fixed — CrudRow given an explicit height(50.dp) (+6.dp spacing = the 56.dp paging estimate) so rows can't grow past the page budget; CrudListPage.kt:122
...
**Status:** Fixed — Copy now clears the model (Mode.Add(item.copy(model = ""))) so the copy can't collapse onto its source; cooldowns/list.kt:55
...
**Status:** Fixed — on edit, when the (ceiling-rounded) hours field is untouched the original absolute availableAtMs is kept instead of recomputed; cooldowns/edit.kt:48,71
...
**Status:** Fixed — upsert now dedups on the (providerId, modelId) composite (drops any other row sharing the saved row's pair); manualoverrides/list.kt:32
...
**Status:** Fixed — added categorySingularName(category) and used it for the View title instead of removeSuffix("s"); InternalPromptsScreen.kt, InternalPromptCrud.kt:92
...
**Status:** Won't fix — touches the guarded two-tier-navigation back logic; per repo guardrails (back-stack regressions shipped 3× before) a rework here is too risky for a MEDIUM edge that only affects deep-link re-entry depth.
...
**Status:** Fixed — null branch now navigates via LaunchedEffect(selectedProviderId) once instead of calling goBack() every frame; SettingsScreen.kt:366
...
**Status:** Not a bug — documented single-editor assumption; the audit itself notes it's usually fine. Keying mirrors on generalSettings would fight the debounce. No external write reaches these while mounted in practice.
...
**Status:** Fixed — covered by Bug 5: appWideSystemPromptId/ParametersIds + reportModelSystemPromptId/ParametersIds now round-trip in the Settings export; ImportExportScreen.kt:265,281
...
**Status:** Won't fix — the code already mitigates with the documented skip-when-already-matches guards (ServiceSettingsScreens.kt:320-331); fully serialising the two debounced effects is a risky rework of the auto-save design for a narrow race.
...
**Status:** Won't fix — the audit notes the silent-drop guard is intentional (good for auto-save); inline per-field validation hints are a UX enhancement, not a correctness fix.
...
**Status:** Fixed — reworded the Knowledge card description to state plain non-URL text can't be ingested (use New Report); ShareChooserScreen.kt:101
...
**Status:** Fixed — else branch now reads "Open the new-report screen" instead of duplicating the select headline; ExternalIntentConfirmScreen.kt:138
...
**Status:** Fixed — unknown non-blank nextAction now renders "• Run action: <nextAction>"; ExternalIntentConfirmScreen.kt:165
...
**Status:** Won't fix — hasTrimmable is computed by the caller (nav host), out of scope; reactive recompute belongs there.
...
**Status:** Fixed — agents Copy now resolves a unique name up front ("-copy", then "-copy 2", …) via uniqueCopyName; agents/list.kt:62
...
**Status:** Fixed — blocked-model Copy now clears the model (Mode.Add(item.copy(model = ""))); blocked/list.kt:55
...
**Status:** Not a bug — cosmetic; "N set" counting field presence is acceptable as the audit notes.
...
**Status:** Won't fix — optional UX enhancement (the audit says "optionally"); adding a dirty-state confirm to every CRUD/edit form's back handler is a broad behavioral change, not a correctness fix.
...
**Status:** Fixed — bare-array branch now derives a modelTypes map (keep existing per-model type, infer for new ids) and uses the 2-arg withModels so types are re-inferred consistently with the object branch (vision/web/reasoning preserved); ImportExportScreen.kt:424
...
**Status:** Fixed — standalone costs CSV importer now counts distinct (provider, model) keys via a HashSet; ImportExportScreen.kt:1133
...
**Status:** Fixed — dup-mode keyMatchesOriginal Save-gate already present; the edit (non-dup) path now prunes the original key on repoint via the isAddMode flag added to onSave (see Bug 4); StatisticsScreen.kt:264,335
...
**Status:** Won't fix — LOW; the format is internally consistent (US dot in / locale-independent dot out). Accepting comma risks ambiguity with thousands separators; a hint is a minor UX nicety, deferred.
...
**Status:** Fixed — for already-present reports we now backfill any secondary whose id isn't already on disk (listForReport diff); ImportExportScreen.kt:189
...
**Status:** Fixed — secondaries whose reportId != report.id are now skipped; ImportExportScreen.kt:189
...
**Status:** Fixed — added isFixedListCategory(category) next to categoryDisplayName; InternalPromptCrud now calls it; InternalPromptsScreen.kt, InternalPromptCrud.kt:42
...
**Status:** Not a bug — acceptable per the audit; category-keyed mode reset is the intended behavior and the in-progress-edit loss is a rare edge.
...
**Status:** Fixed — defaultTypePaths now read with TypeTokens.mapStringStringType instead of the unchecked Map::class.java cast; SettingsPreferences.kt:48
...
**Status:** Not a bug — verified: the shared "cost_config" HelpContent (SettingsAdminHelp.kt:620) explicitly documents the Cleanup + Layered-costs maintenance cards that CostsMaintenanceScreen shows, so the topic matches the screen.
...
**Status:** Fixed — Agent view Model field now appends "(provider default)" when agent.model is blank (inherited); agents/view.kt:32
...
**Status:** Won't fix — the confirm dialog already warns explicitly ("Drops every … including any you customized"); merge-vs-replace / export-first is a feature request, not a correctness bug.
...
**Status:** Won't fix — LOW; imports work and the resolver falls back gracefully on dangling refs. A referential-integrity warning across the worker graph is an enhancement, deferred.
...
**Status:** Won't fix — LOW; same class as Bug 54. The resolver falls back silently and imports succeed; a dependency warning is an enhancement, deferred.
...
**Status:** Not a bug — the breadth is necessary (providers mis-report zip mime) and the manifest check guards bad picks; the audit itself marks it acceptable.
...
**Status:** Fixed — sanitised title is now trim('_','.','-')'d before ifBlank so an all-separator (CJK/emoji) title falls back to "report"; ImportExportScreen.kt:785
...
**Status:** Fixed — moot now that Bug 26 prevents same-(provider,model) duplicates; manualoverrides/list.kt:32
...
**Status:** Fixed — edit branch now uses effectiveCategory = ip?.category ?: selectedInternalCategory for both fixedCategory and the existingNames filter; SettingsScreen.kt:655
...
**Status:** Fixed — Costs and Test cards now gated behind hasActiveProvider; HousekeepingScreen.kt:74
...
**Status:** Fixed — LiteLLM picks now held in local pendingEndpoints (shown in the dropdown) and only persisted via onAddEndpoint when the agent is saved, for the actually-selected endpoint; AgentsScreen.kt:81,160,240
...
**Status:** Won't fix — the Edit-mode copy path already blocks renaming to the source (existingNames includes all agents since the copy's new id isn't in the list), and Bug 38's unique-copy-name now prevents the initial collision; the residual is a narrow double-duplicate edge. Structural reroute deferred.
...
**Status:** Won't fix — LOW; the tile-order write happens during the apply/commit phase alongside the other singleton writes (costs/cooldowns/registry), so it's no worse than them. Returning it through the result is a signature refactor disproportionate to the impact.
...
**Proposed fix:** Cosmetic; acceptable.
**Status:** Not a bug — cosmetic 4-digit cap; acceptable as the audit notes.
...
**Status:** Fixed — changed the All-import save gate from identity (!==) to value inequality (!=); ImportExportScreen.kt:1549
...
**Status:** Not a bug — the maxOf(1, …) coerce prevents the subList out-of-bounds; edge UX only, acceptable as the audit notes.
