...
**Status:** Fixed — registered NullSafeFieldAdapterFactory in createAppGson that coerces null non-null String/List/Set/Map fields to empty defaults (ApiModels.kt:~33-100); covers reports, settings, sessions, KB manifests, seeds.
...
**Status:** Fixed — only throw truncation IOException when no content chunk was emitted; a clean EOF after content is now valid for socket-close-without-[DONE] providers (ApiStreaming.kt:~132).
...
**Status:** Won't fix — spec-conformant for well-formed streams; hardening is speculative for non-standard interleaving.
...
**Status:** Won't fix — downstream classification matches on the 'API error: <code>' message string (IconGenerationManager/ModelTestEngine); a typed exception would still need that string, churn risks regressions.
...
**Status:** Fixed — chatGemini now joins all candidate text parts (flatMap parts → joinToString) instead of parts[0] only (ApiDispatch.kt:~582).
...
**Status:** Fixed — extractGeminiContent joins all parts' text per chunk instead of firstOrNull (ApiStreaming.kt:~187).
...
**Status:** Won't fix — fix belongs in ui/ (surfacing effective max_tokens); the floor itself is required for Anthropic correctness.
...
**Status:** Not a bug — refuse-and-surface on empty/dim-mismatch vector is intentional (documented in the inline comment); silently dropping chunks would create invisible data loss.
...
**Status:** Won't fix — fix belongs in viewmodel/AppViewModel.refreshAllModelLists (success keyed on ids.size>0); empty-but-valid is rare and low-impact.
...
**Status:** Won't fix — raw-snapshot tee instead of a second GET is a larger ApiClient/interceptor change; LOW value vs risk.
...
**Status:** Fixed — testApiConnectionWithJson now parses the success body by service.apiFormat (Gemini/Anthropic/OpenAI) instead of assuming OpenAiResponse (ApiDispatch.kt:~979).
...
**Status:** Fixed — TagPropagatingExecutor now sets/restores permitPreAcquired + suppressInlineRetry unconditionally so a stale true can't leak onto a pooled worker (TagPropagation.kt:~141-151).
...
**Status:** Fixed — read the 64KiB error-body peek once and pass the decoded string into all four predicates instead of peeking four times (RateLimitRetry.kt:~82, helper signatures).
...
**Status:** Fixed — guard the jitter math against expBackoff<=0 (RateLimitRetry.kt:~135).
...
**Status:** Won't fix — Thread.sleep in interceptor is acceptable per cached-thread-pool design; comment already notes the suspend alternative.
...
**Status:** Fixed (a) — 529 retry now uses exponential-with-jitter backoff mirroring the 429 path (OverloadedRetry.kt:~44). (b) lastTraceFilename reset left out deliberately: 529 path doesn't read it and a reset could interfere with the 429 bench logic.
...
**Status:** Fixed — added redactBody pass that masks key-bearing JSON fields in the persisted request body (TracingInterceptor.kt:~50, BODY_KEY_FIELD_REGEX).
...
**Status:** Won't fix — heuristic is acceptable; consulting the request stream flag is speculative for misbehaving proxies.
...
**Status:** Fixed — corrected '[partitial:' → '[partial:' (TracingInterceptor.kt:~177).
...
**Status:** Fixed — disk write (writeTextAtomic) now happens outside the lock; the lock only guards the in-memory cache mutation (ApiTracer.kt:~104-145).
...
**Status:** Fixed (a) — seed fileSequence with a random per-process offset so a post-restart same-ms/same-host trace can't collide on filename (ApiTracer.kt:~32). (b) full-resort cost left as-is (insertion-sort refactor risk on the update path).
...
**Status:** Won't fix — re-serialize on rare miss-tag is acceptable; sidecar is over-engineering for the frequency.
...
**Status:** Fixed — unique per-writer tmp name '<name>.<uuid>.tmp' so concurrent same-dest writers don't share a staging file (AtomicFileWrite.kt:~21).
...
**Status:** Won't fix — no portable directory-fsync API on Android; low practical impact on internal ext4 storage (documented).
...
**Status:** Fixed — loadSession/deleteSession now apply the isSafeFlatId + canonical-containment guard via isSafeSessionFile (ChatHistoryManager.kt).
...
**Status:** Fixed — notifyHistoryChanged uses a monotonic counter (_historyVersion.update{it+1}) instead of wall-clock (ChatHistoryManager.kt:~172).
...
**Status:** Won't fix — the single global lock is correctness-critical for the additive cost logic; per-report striping / in-memory model is a large architectural change, too risky here.
...
**Status:** Fixed — totalCost now recomputed whenever any of cost/inputCost/outputCost changed, not only when primary cost!=null (ReportStorage.kt:~111).
...
**Status:** Won't fix — metadata cache for getAllReports is a larger design change; fix deferred (performance-only).
...
**Status:** Fixed (via Bug 1) — Gson null-safety factory coerces src.title to "" if missing, so copyReport no longer NPEs.
...
**Status:** Won't fix — unconditional cost accumulation is intentional and documented (callers use save() to overwrite without accumulating); runId-marker change is a product-level behavior change.
...
**Status:** Fixed — list/get/bump/count paths now resolve via resolveReportDirForRead which applies the same traversal guard as the write path (SecondaryResult.kt).
...
**Status:** Fixed — countByMetaName now builds its tally from listForReport (shares the fingerprint cache) instead of re-parsing every file (SecondaryResult.kt:~679).
...
**Status:** Won't fix — single-occurrence expansion + greedy whitespace is the documented intent; changing it risks altering existing fan-in prompt formatting.
...
**Status:** Fixed — lookupPricing now places the manual OVERRIDE before the curated tiers, matching getPricing's precedence (PricingCache.kt:~426).
...
**Status:** Fixed — modelsDev/helicone/llmPrices/aa tiers now memoized as emptyMap()/emptyList() after a missing load, so the ==null guard short-circuits and stops re-running loadBlob per getPricing (PricingCache.kt:~1313-1368).
...
**Status:** Won't fix — getAllPricing's litellm+openrouter+manual merge predates the seven-tier chain; reworking it is a display-only change with consumer-side risk.
...
**Status:** Fixed — zero-baseline apiCost split now uses the token ratio instead of attributing 100% to output (PricingCache.kt:~297).
...
**Status:** Fixed — createKnowledgeBase + renameKnowledgeBase now take the shared lock around the manifest write/read-modify-write (Knowledge.kt:~151-162).
...
**Status:** Fixed — saveSource/deleteSource now wrap loadKb in runCatching and bail instead of throwing out of the public API (Knowledge.kt:~207,258).
...
**Status:** Won't fix — fix belongs partly in ui/ (surface error on the source row); refusing the save here would also need a UI signal — deferred to product/UI.
...
**Status:** Fixed — if the top-ranked hit alone exceeds maxContextChars it's now truncated to fit rather than skipped while smaller chunks fill the budget (KnowledgeService.kt:~283).
...
**Status:** Won't fix — per-embedder-group querying / user-visible warning belongs in ui/; LOW.
...
**Status:** Won't fix — post-download content validation is a reasonable hardening but speculative (server returning 200 HTML as .tflite); LOW, deferred.
...
**Status:** Not a bug — documented callback contract (total may be -1); UI handles it. No code change needed.
...
**Status:** Won't fix — MediaPipe LlmInference has no cancel/streaming API; documented limitation. A timeout wrapper is a larger change, deferred.
...
**Status:** Fixed — cooldownCaption now compares year AND day-of-year for the same-day check (ModelCooldownStore.kt:~149).
...
**Status:** Fixed — isUnavailable is now a pure timestamp compare with no remove/persist/publish side effects; expiry pruning stays in init/pruneExpired (ModelCooldownStore.kt:~88).
...
**Status:** Won't fix — get()'s stale-delete vs getRaw's longer TTL is a niche window; separating into a sweep is a behavior change, LOW.
...
**Status:** Fixed (via Bug 23) — unique tmp filename removes the shared-<name>.tmp race for concurrent same-key puts.
...
**Status:** Won't fix — clear-then-restore is intentional (clean restore state); project policy is no forward/backward-compat merge (CLAUDE.md). Process is force-killed after restore.
...
**Status:** Won't fix — validate-then-write staging is intentional; two-pass streaming refactor is large and risks the validation guarantee. The 1GB cap bounds it.
...
**Status:** Fixed — null/non-string 's' prefs values are now skipped instead of putString(k,null) removing the key (BackupManager.kt:~508).
...
**Status:** Fixed — refreshAllModelLists uses _uiState.updateAndGet to persist the exact post-update snapshot per provider instead of re-reading _uiState.value (AppViewModel.kt:~2020).
...
**Status:** Not a bug — length/4 is an acceptable stats heuristic (documented); provider-reported usage is preferred where available.
...
**Status:** Won't fix — GlobalScope+NonCancellable survival flush is intentional and documented; prefs commits are internally serialized.
...
**Status:** Fixed — markAgentRunningAsync now called with named requestBody=aiPrompt so the prompt lands in requestBody not requestHeaders (ReportViewModel.kt:~604).
...
**Status:** Fixed — mutateJob now uses RegenerateBatchStorage.update (compound get→mutate→save under one lock) so a concurrent cancel can't be clobbered by a stale-snapshot save (RegenerateBatchEngine.kt:~598).
...
**Status:** Won't fix — fully-atomic start guard across reconcile/restart needs a larger restructure; with Bug 80 fixed the activeSecondaryBatches counter stays balanced even on a double-start window. LOW.
...
**Status:** Won't fix — fix belongs in the engine's pause attribution policy (null pausedOnRowId) but touches Restart-gating semantics; LOW, deferred to avoid behavior change.
...
**Status:** Won't fix — long-held fairness mutex over the poll loop is the documented FIFO design intent.
...
**Status:** Won't fix — Semaphore(1) fallback is a defensive guard; building the host key once + asserting is a refactor, LOW. Could add a log but not behavior-critical.
...
**Status:** Won't fix — combined system+context budget reconciliation needs model-window awareness / a truncation-policy product decision; LOW.
...
**Status:** Fixed (via Bug 12) — the underlying TagPropagatingExecutor leak is fixed, so the test-engine suppressInlineRetry no longer leaks onto later calls.
...
**Status:** Not a bug — rate-window-before-concurrency is the documented 'safe direction' (over-throttle, never under).
...
**Status:** Not a bug — semaphore rebuild transient over-cap is documented and accepted.
...
**Status:** Fixed — fetchUrlAsString now logs at WARN on non-2xx and on exception instead of silently returning null (ApiClient.kt:~262).
...
**Status:** Not a bug — fragility note only; all current call paths flow through the interceptor stack. No code change.
...
**Status:** Not a bug — process-wide kind caps with no per-report sub-allocation is documented/accepted design.
...
**Status:** Fixed — formatHeaders now redacts key-bearing headers (Authorization/x-api-key/etc.) before they persist in the report agent JSON (AnalysisRepository.kt:~104).
...
**Status:** Won't fix — AND-merge of default-true returnCitations is documented in the inline comment; a tri-state is a data-model change, LOW.
...
**Status:** Fixed (via Bug 23) — unique tmp filename removes the shared-<name>.tmp race for concurrent same-provider saves.
...
**Status:** Fixed — added RegenerateBatchStorage.update(reportId){...} that holds the lock across read-modify-write (RegenerateBatchStorage.kt:~69).
...
**Status:** Fixed (via Bug 12) — TagPropagatingExecutor leak fix removes the permitPreAcquired leak exposure for the translation fan-out too.
...
**Status:** Won't fix — synchronous SettingsHolder.current publish at every aiSettings mutation site is broad and error-prone; the staleness window is tiny. LOW.
...
**Status:** Fixed (via Bug 1) — the createAppGson null-safety factory covers Settings/Agent/AgentParameters non-null fields too.
...
**Status:** Not a bug — all five seed loaders already log on parse/read failure (WARN); Bug 1's Gson fix covers the null-field concern.
...
**Status:** Won't fix — joinToString("") across text blocks is the existing convention; inserting boundary whitespace risks altering well-formed web-search answers. LOW.
...
**Status:** Fixed — buildChatUrl now requires a '/' boundary (endsWith("/$cleanedChatPath")), dropping the bare-substring endsWith branch (ApiDispatch.kt:~1135).
...
**Status:** Fixed — cancel() no longer decrements activeSecondaryBatches directly; only the orchestrator job's finally decrements, removing the double-decrement (RegenerateBatchEngine.kt:~154).
