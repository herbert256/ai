...
files — the 24 files under `viewmodel/`). There are **50
cloud providers** loaded at runtime from one JSON file per provider
under `assets/providers/` (47 `OPENAI_COMPATIBLE`, 1 `ANTHROPIC`, 1
...
### Reference data
- **[providers.md](providers.md)** — All 50 cloud providers from the
...
- `assets/providers/` — provider definitions, one JSON file per
  provider (50 files, each a bare `ProviderDefinition` object — no
  `{ "providers": [...] }` wrapper)
