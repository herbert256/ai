...
files — the 24 files under `viewmodel/`). There are **47
cloud providers** loaded at runtime from one JSON file per provider
under `assets/providers/` (45 `OPENAI_COMPATIBLE`, 1 `ANTHROPIC`, 1
...
### Reference data
- **[providers.md](providers.md)** — All 47 cloud providers from the
...
- `assets/providers/` — provider definitions, one JSON file per
  provider (47 files, each a bare `ProviderDefinition` object — no
  `{ "providers": [...] }` wrapper)
