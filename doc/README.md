...
Kotlin 2.4.0 + Jetpack Compose, ~161,212 LOC across 406 Kotlin files
(`data` 97, `ui` 279, `viewmodel` 27, `model` 2, plus the one entry
...
managers (plus the `BatchEngine` / `SecondaryBatchEngine` bases, a
`WorkerRunner`, and type/support files — the 27 files under
`viewmodel/`). There are **91
...
### For developers
- **[Functional audit reassessment](functional-audit-reassessment-2026-09-07.md)** — Current status of all 14 functional findings after both Report remediation rounds; [HTML edition](../functional-audit-2026-09-07.html).
...
- **[api-formats.md](api-formats.md)** — The four API dispatch paths
  (`OPENAI_COMPATIBLE`, `ANTHROPIC`, `GOOGLE`, `REPLICATE`) and what's
  quirky about each.
...
- **[tournament-judges-compare.md](tournament-judges-compare.md)** —
  Worker-judged report analysis: Tournament rankings via seven methods
  (Copeland win-rate now over per-model contested games, plus ELO,
  Davidson, Markov, Schulze, Colley, TrueSkill2), Judge-the-judges
  agreement, and Compare-with-meta
  similarity grids.
...
- **[help.md](help.md)** — The in-app Help system: the white ❔ live
  "&lt;screen&gt; - icons" overlay (every screen with a bottom icon bar,
  not just report-Manage — the View family shows only the red ❓) vs
  the red ❓ help page, per-screen topics, per-provider /
  per-repository pages, and the static icon-table legend.
...
- **[log-details.md](log-details.md)** — Generated reference of
  ...
  grouped by severity (ERROR / WARN / INFO / DEBUG) then
...
- `assets/providers/` — provider definitions, one JSON file per
  provider (91 files, each a bare `ProviderDefinition` object — no
  `{ "providers": [...] }` wrapper)
- `assets/internal-prompts/` — Internal Prompts (Meta / Fan-out / Fan-in / Other internal)
- `assets/prompts/examples/` — Example Prompts library (one JSON file per prompt)
- `data/AppService.kt` — provider runtime model
- `data/ApiFormat.kt` + `data/ApiDispatch.kt` (split across
  `ApiDispatchModels.kt` / `ApiDispatchStreaming.kt` /
  `ApiDispatchBuilders.kt` / `ReplicateDispatch.kt`) — dispatch routing
- `data/PricingCache.kt` — layered pricing + capability lookup. The
  ...
  LiteLLM → models.dev → llm-prices → Artificial Analysis → llm-stats →
  OpenRouter cross-provider fallback → Requesty → genai-prices →
  TrueFoundry → Helicone → `DEFAULT_PRICING`
  ($25/M in, $75/M out). Manual override sits **above** all curated
  catalog tiers but below the two provider self-report tiers. (Both the
  class-level KDoc and `getPricing`'s own KDoc are stale — the
  class-level comment still omits genai-prices/TrueFoundry and
  `getPricing`'s says "five-tier lookup" — the code above is
  authoritative.) Tier blobs live
...
- `data/preferences/SettingsPreferences.kt` — every prefs key
