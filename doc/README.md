...
files — the 24 files under `viewmodel/`). There are **48
...
### Reference data
- **[providers.md](providers.md)** — All 48 cloud providers from the
...
- `assets/providers/` — provider definitions, one JSON file per
  provider (48 files, each a bare `ProviderDefinition` object — no
  `{ "providers": [...] }` wrapper)
