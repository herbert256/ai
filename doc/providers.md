...
— 49 files, each a bare `ProviderDefinition` object (no
...
**49 providers total** — 46 `OPENAI_COMPATIBLE`, 1 `ANTHROPIC`
(Anthropic), 1 `GOOGLE` (Google), 1 `REPLICATE` (Replicate). All 46
...
`ApiFormat.kt` is stale — the real OpenAI-compatible count is 46.)
